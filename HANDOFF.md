...
- `2026-07-01`: 자동 발행 모드(`BLOGGER_REQUIRE_REVIEW_APPROVAL=false`)가 `platform-publish-plan`에도 반영되도록 수정했습니다. 이제 `user_final_confirmation_required=false`, `approved_ready_count=12`, Blogger `ready_item_count=12`로 보이며, 실제 업로드는 별도 cap으로 제한됩니다.
- `2026-07-01`: 사용자가 방문자 확대를 위해 품질과 양을 함께 늘리라고 지시했으므로 GitHub Actions Blogger 기본 cap을 `BLOGGER_MAX_POSTS_PER_RUN=3`으로 상향했습니다. 단, `pre-publish-quality-gate` 통과, 동일 본문 skip, slug 안정성 보호는 유지합니다.
...
- 오늘 추가 공개 URL:
  ...
  - `국제유가 전망: 지금 투자자가 확인할 체크포인트 5가지` -> `https://gimu-economy-insight.blogspot.com/2026/07/5_097860030.html`

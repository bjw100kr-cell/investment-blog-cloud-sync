...
- freshness_summary: 최신 근거가 살아 있어 데일리 해설로 다루기 좋은 상태입니다. 대표 근거: Bitcoin stalls as open interest decline raises questions about rally's staying power
...
- `중국 변수와 시장 영향: 환율, 경기부양, 원자재를 같이 봐야 하는 이유` / keyword `china` / ready `False` / quality `review_before_publish` / freshness `fresh`

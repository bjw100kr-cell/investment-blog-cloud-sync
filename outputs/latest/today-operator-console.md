...
- `유가 상승이 물가와 증시에 번지는 경로: 투자자가 볼 3가지` / keyword `oil` / verdict `approve` / publish `2026-07-04`
- `비트코인 가격보다 먼저 봐야 할 것: ETF 자금, 달러, 규제 체크포인트` / keyword `bitcoin` / verdict `approve` / publish `2026-07-05`
- `AI 반도체 주가를 볼 때 실적보다 먼저 확인할 3가지` / keyword `ai_semiconductors` / verdict `approve` / publish `2026-07-06`
...
- `ai_semiconductors` 예상 `83`명: AI 반도체 주가를 볼 때 실적보다 먼저 확인할 3가지
- `oil` 예상 `74`명: 유가 상승이 물가와 증시에 번지는 경로: 투자자가 볼 3가지
- actual_verified: `0`
...
- `main_post` / `유가 상승이 물가와 증시에 번지는 경로: 투자자가 볼 3가지` / 페이지뷰와 체류시간 균형 확보

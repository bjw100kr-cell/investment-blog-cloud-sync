...
- `비트코인 가격보다 먼저 봐야 할 것: ETF 자금, 달러, 규제 체크포인트` / keyword `bitcoin` / verdict `approve` / publish `2026-08-02`
- `FOMC 이후 시장, 주식과 코인이 같이 흔들리는 이유와 확인할 3가지` / keyword `fomc` / verdict `approve` / publish `2026-08-01`
...
- `treasury_yields` 예상 `90`명: 미국채 금리 상승이 나스닥과 코인에 부담이 되는 이유
- actual_verified: `0`
...
- `next_slot` / `AI 반도체 주가를 볼 때 실적보다 먼저 확인할 3가지` / 카테고리 보강과 내부링크 확장으로 장기 검색 유입 기반을 만드는 역할

...
- `비트코인 가격보다 먼저 봐야 할 것: ETF 자금, 달러, 규제 체크포인트` / keyword `bitcoin` / verdict `approve` / publish `2026-08-10`
- `미국 증시 지수 흐름: 나스닥, 금리, 빅테크 실적을 같이 봐야 하는 이유` / keyword `us_index_flow` / verdict `approve` / publish `2026-08-11`
- `중국 변수와 시장 영향: 환율, 경기부양, 원자재를 같이 봐야 하는 이유` / keyword `china` / verdict `approve` / publish `2026-08-12`
...
- `us_index_flow` 예상 `80`명: 미국 증시 지수 흐름: 나스닥, 금리, 빅테크 실적을 같이 봐야 하는 이유
- `china` 예상 `50`명: 중국 변수와 시장 영향: 환율, 경기부양, 원자재를 같이 봐야 하는 이유
- actual_verified: `0`
...
- `next_slot` / `미국 증시 지수 흐름: 나스닥, 금리, 빅테크 실적을 같이 봐야 하는 이유` / 페이지뷰와 체류시간 균형 확보

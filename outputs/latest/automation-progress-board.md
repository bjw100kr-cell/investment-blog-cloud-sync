...
- `measurement_and_growth` / status `in_progress` / owner `agent`
  ...
  - proof: search_console_ready `True`
  - proof: openai_ready `False`
  - proof: monetization_score `50.0`
  - next_action: Search Console site URL, GA4, AdSense 연결을 순차적으로 보강
- `wordpress_expansion` / status `blocked` / owner `user`
  - summary: WordPress는 2차 확장 채널이며 현재 필수 자격값이 없어 막혀 있습니다.
  - proof: wordpress_ready `False`
  - proof: missing `WORDPRESS_SITE_URL, WORDPRESS_USERNAME, WORDPRESS_APPLICATION_PASSWORD`

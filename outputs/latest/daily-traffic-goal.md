...
3. `treasury_yields` 미국채 금리 상승이 나스닥과 코인에 부담이 되는 이유 / 예상 `93`명 / 수요 `3500` / 품질 `pass`
   - action: 금리·달러·주식·코인 영향까지 한 번에 설명하는 evergreen 허브로 연결
4. `ai_semiconductors` AI 반도체 주가를 볼 때 실적보다 먼저 확인할 3가지 / 예상 `58`명 / 수요 `0` / 품질 `pass`
...
- `treasury_yields` lane `macro` / 예상 `93`명 / ready `True` / quality `pass`

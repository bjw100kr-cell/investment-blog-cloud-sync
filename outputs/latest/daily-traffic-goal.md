...
1. `fomc` FOMC 이후 시장, 주식과 코인이 같이 흔들리는 이유와 확인할 3가지 / 예상 `94`명 / 수요 `3500` / 품질 `pass`
   - action: 금리·달러·주식·코인 영향까지 한 번에 설명하는 evergreen 허브로 연결
2. `bitcoin` 비트코인 가격보다 먼저 봐야 할 것: ETF 자금, 달러, 규제 체크포인트 / 예상 `68`명 / 수요 `400` / 품질 `pass`
   - action: 코인 시장 신호와 ETF/규제 후속 글을 내부링크로 묶어 발행
3. `ai_semiconductors` AI 반도체 주가를 볼 때 실적보다 먼저 확인할 3가지 / 예상 `57`명 / 수요 `0` / 품질 `pass`
   - action: 대표 종목/실적/지수 흐름 후속 글로 페이지뷰 확장
4. `tariffs_trade` 관세와 무역 갈등이 증시에 미치는 영향: 환율과 공급망까지 보기 / 예상 `50`명 / 수요 `0` / 품질 `pass`
...
- `fomc` lane `macro` / 예상 `94`명 / ready `True` / quality `pass`
- `bitcoin` lane `crypto` / 예상 `68`명 / ready `True` / quality `pass`
  - crypto signal: BTCUSDT 24h change 7.30%
- `ai_semiconductors` lane `us-stocks` / 예상 `57`명 / ready `True` / quality `pass`
- `tariffs_trade` lane `world-flow` / 예상 `50`명 / ready `True` / quality `pass`

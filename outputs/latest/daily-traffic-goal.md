...
1. `ai_semiconductors` AI 반도체 주가를 볼 때 실적보다 먼저 확인할 3가지 / 예상 `95`명 / 수요 `3900` / 품질 `pass`
...
3. `fomc` FOMC 이후 시장, 주식과 코인이 같이 흔들리는 이유와 확인할 3가지 / 예상 `94`명 / 수요 `3500` / 품질 `pass`
   - action: 금리·달러·주식·코인 영향까지 한 번에 설명하는 evergreen 허브로 연결
4. `china` 중국 변수와 시장 영향: 환율, 경기부양, 원자재를 같이 봐야 하는 이유 / 예상 `76`명 / 수요 `2400` / 품질 `pass`
...
- `china` lane `world-flow` / 예상 `76`명 / ready `True` / quality `pass`

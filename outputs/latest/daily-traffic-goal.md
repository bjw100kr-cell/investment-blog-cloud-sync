...
1. `bitcoin` 비트코인 가격보다 먼저 봐야 할 것: ETF 자금, 달러, 규제 체크포인트 / 예상 `95`명 / 수요 `6000` / 품질 `pass`
...
3. `china` 중국 변수와 시장 영향: 환율, 경기부양, 원자재를 같이 봐야 하는 이유 / 예상 `71`명 / 수요 `2000` / 품질 `pass`
...
- `china` lane `world-flow` / 예상 `71`명 / ready `True` / quality `pass`
- `ai_semiconductors` lane `us-stocks` / 예상 `0`명 / ready `True` / quality `needs_fix`

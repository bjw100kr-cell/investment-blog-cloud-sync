...
1. `fomc` FOMC 이후 시장, 주식과 코인이 같이 흔들리는 이유와 확인할 3가지 / 예상 `95`명 / 수요 `6600` / 품질 `pass`
   - action: 금리·달러·주식·코인 영향까지 한 번에 설명하는 evergreen 허브로 연결
2. `china` 중국 변수와 시장 영향: 환율, 경기부양, 원자재를 같이 봐야 하는 이유 / 예상 `50`명 / 수요 `0` / 품질 `pass`
   - action: 세계 흐름 해설 뒤 관련 섹터/환율 글로 연결

## 병목

- 상위 4개 글 예상 합계가 145명으로 목표보다 55명 부족합니다.
- GA4/Search Console 연결 전이라 실제 200명 달성 여부를 자동 측정하기 어렵습니다.
- 뉴스레터/텔레그램 재방문 동선이 없어 첫 방문자를 반복 방문으로 쌓기 어렵습니다.
- 오늘 바로 발행할 Blogger ready item이 없습니다.
...
- `bitcoin` lane `crypto` / 예상 `0`명 / ready `True` / quality `review_before_publish`

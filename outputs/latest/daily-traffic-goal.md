...
1. `fomc` FOMC 이후 시장, 주식과 코인이 같이 흔들리는 이유와 확인할 3가지 / 예상 `95`명 / 수요 `5700` / 품질 `pass`
...
3. `china` 중국 변수와 시장 영향: 환율, 경기부양, 원자재를 같이 봐야 하는 이유 / 예상 `54`명 / 수요 `0` / 품질 `pass`
...
- `bitcoin` lane `crypto` / 예상 `95`명 / ready `True` / quality `pass`
  - crypto signal: BTCUSDT 24h change -3.51%

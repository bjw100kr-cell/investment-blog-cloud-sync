...
1. `bitcoin` 비트코인 가격보다 먼저 봐야 할 것: ETF 자금, 달러, 규제 체크포인트 / 예상 `95`명 / 수요 `6500` / 품질 `pass`
...
3. `china` 중국 변수와 시장 영향: 환율, 경기부양, 원자재를 같이 봐야 하는 이유 / 예상 `75`명 / 수요 `2300` / 품질 `pass`
   - action: 세계 흐름 해설 뒤 관련 섹터/환율 글로 연결
4. `ai_semiconductors` AI 반도체 주가를 볼 때 실적보다 먼저 확인할 3가지 / 예상 `66`명 / 수요 `300` / 품질 `pass`
...
- `ai_semiconductors` lane `us-stocks` / 예상 `66`명 / ready `True` / quality `pass`

...
3. `ai_semiconductors` AI 반도체 주가를 볼 때 실적보다 먼저 확인할 3가지 / 예상 `89`명 / 수요 `3200` / 품질 `pass`
...
- `bitcoin` lane `crypto` / 예상 `95`명 / ready `True` / quality `pass`
  - crypto signal: BTCUSDT 24h change -2.12%
- `fomc` lane `macro` / 예상 `90`명 / ready `True` / quality `pass`
- `ai_semiconductors` lane `us-stocks` / 예상 `89`명 / ready `True` / quality `pass`

...
2. `ai_semiconductors` AI 반도체 주가를 볼 때 실적보다 먼저 확인할 3가지 / 예상 `86`명 / 수요 `2800` / 품질 `pass`
   - action: 대표 종목/실적/지수 흐름 후속 글로 페이지뷰 확장
3. `bitcoin` 비트코인 가격보다 먼저 봐야 할 것: ETF 자금, 달러, 규제 체크포인트 / 예상 `69`명 / 수요 `700` / 품질 `pass`
...
- `ai_semiconductors` lane `us-stocks` / 예상 `86`명 / ready `True` / quality `pass`
- `bitcoin` lane `crypto` / 예상 `69`명 / ready `True` / quality `pass`

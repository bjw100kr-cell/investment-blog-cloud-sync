...
- pick: `유가 상승이 물가와 증시에 번지는 경로: 투자자가 볼 3가지` / `oil` / `main_pick` / ready `False` / 지금 이 클러스터를 대표하는 메인 글
...
- pick: `국제유가 전망이 주식과 코인에 미치는 영향` / `seo_oil_7` / `followup_pick_1` / ready `True` / 검색형 유입을 받기 좋은 주제
...
- pick: `국제유가 전망에서 다음으로 봐야 할 체크포인트 5가지` / `seo_oil_8` / `followup_pick_2` / ready `True` / 재방문과 내부링크 순환에 유리한 주제
...
- pick: `AI 반도체 주식 공급망 정리: 누가 수혜를 보나` / `seo_ai_semiconductors_11` / `followup_pick_2` / ready `True` / 페이지 체류시간을 늘리기 좋은 주제

...
- `fomc` / FOMC 이후 시장, 주식과 코인이 같이 흔들리는 이유와 확인할 3가지 / lane `macro` / publish 2026-08-26 / priority 139.0 / freshness `` / quality `pass` / hero_image_selected `True`
- `bitcoin` / 비트코인 가격보다 먼저 봐야 할 것: ETF 자금, 달러, 규제 체크포인트 / lane `crypto` / publish 2026-08-27 / priority 121.0 / freshness `fresh` / quality `pass` / hero_image_selected `True`
...
- why_not_other_topics:
  - `seo_pce_7`도 같은 `macro` 레인이지만 현재 점수와 발행 준비도 기준에서는 `fomc`가 앞섭니다.
  - `seo_cpi_8`도 같은 `macro` 레인이지만 현재 점수와 발행 준비도 기준에서는 `fomc`가 앞섭니다.
  - `seo_treasury_yields_9`도 같은 `macro` 레인이지만 현재 점수와 발행 준비도 기준에서는 `fomc`가 앞섭니다.

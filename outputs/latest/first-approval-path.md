...
- item_count: `2`
- ready_now_count: `1`
- user_confirmation_command: `python3 /home/runner/work/investment-blog-cloud-sync/investment-blog-cloud-sync/scripts/set_review_approvals.py --keywords bitcoin cpi`
- `bitcoin` / 비트코인 가격보다 먼저 봐야 할 것: ETF 자금, 달러, 규제 체크포인트 / lane `crypto` / publish 2026-08-17 / priority 122.0 / freshness `fresh` / quality `pass` / hero_image_selected `True`
- `cpi` / CPI 발표 후 금리와 나스닥, 비트코인이 같이 움직이는 이유 / lane `macro` / publish 2026-08-16 / priority 102.0 / freshness `` / quality `review_before_publish` / hero_image_selected `True`
...
- why_not_other_topics:
  ...
  - `cpi`는 freshness는 괜찮아도 quality `review_before_publish` 상태라 바로 올리기보다 추가 검토가 먼저입니다.

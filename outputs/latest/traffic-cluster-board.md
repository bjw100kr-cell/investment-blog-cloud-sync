...
- followup: `미국 빅테크 주식 관련 대표 종목 한눈에 보기` / `seo_us_big_tech_11` / priority `87.5` / 섹터형 검색 유입 누적

## 3. treasury_yields 클러스터

- source_keyword: `treasury_yields`
- revenue_priority_rank: `99`
- cluster_priority_score: `481.5`
...
- main_html_path: `/home/runner/work/investment-blog-cloud-sync/investment-blog-cloud-sync/outputs/latest/publish-ready/04-미국채-금리-상승이-나스닥과-코인에-부담이-되는-이유.html`
- followup: `미국채 금리 상승이 나스닥과 코인에 부담이 되는 이유가 주식과 코인에 미치는 영향` / `seo_treasury_yields_4` / priority `122.5` / 검색 유입 누적과 첫 방문 독자 확보
- followup: `미국채 금리 상승이 나스닥과 코인에 부담이 되는 이유에서 다음으로 봐야 할 체크포인트 5가지` / `seo_treasury_yields_6` / priority `119.5` / 재방문과 내부링크 순환 강화
- followup: `미국채 금리 상승이 나스닥과 코인에 부담이 되는 이유 초보자 가이드: 용어부터 시장 반응까지` / `seo_treasury_yields_8` / priority `116.5` / 광범위한 초보 검색 수요 흡수

## 4. 거시 이벤트 해설 클러스터

- source_keyword: `fomc`
- revenue_priority_rank: `99`
- cluster_priority_score: `129.0`
...
- followup: `달러 인덱스 보는법: 지금 투자자가 확인할 체크포인트 5가지` / `seo_dollar_10` / priority `108.17` / 새 검색 수요를 빠르게 받아내고 내부링크로 기존 핵심 글과 연결

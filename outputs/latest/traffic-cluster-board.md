...
- followup: `중국 변수와 시장 영향 관련 대표 종목 한눈에 보기` / `seo_china_12` / priority `78.5` / 섹터형 검색 유입 누적
- followup: `중국 변수와 시장 영향 공급망 정리: 누가 수혜를 보나` / `seo_china_14` / priority `75.5` / 체류시간과 페이지뷰 확대
- followup: `중국 변수와 시장 영향 ETF·지수·대표 기업 정리` / `seo_china_15` / priority `72.5` / 광고 노출과 장기 검색 유입 확보
...
- followup: `미국채 금리 상승 이유: 지금 투자자가 확인할 체크포인트 5가지` / `seo_treasury_yields_8` / priority `112.07` / 새 검색 수요를 빠르게 받아내고 내부링크로 기존 핵심 글과 연결

## 6. 인플레이션 해설 클러스터

- source_keyword: `cpi`
- revenue_priority_rank: `99`
- cluster_priority_score: `106.1`
- main_title: ``
- main_quality_status: `unknown`
- main_ready_to_upload: `False`
- followup_count: `1`
...
- next_action: 사용자 검토 후 승인 대기
- blocker: main_quality=unknown
- followup: `cpi: 지금 투자자가 확인할 체크포인트 5가지` / `seo_cpi_9` / priority `106.07` / 새 검색 수요를 빠르게 받아내고 내부링크로 기존 핵심 글과 연결

...
- main_html_path: `/home/runner/work/investment-blog-cloud-sync/investment-blog-cloud-sync/outputs/latest/publish-ready/04-유가-상승이-물가와-증시에-번지는-경로-투자자가-볼-3가지.html`
- followup: `국제유가 전망이 주식과 코인에 미치는 영향` / `seo_oil_7` / priority `94.5` / 검색 유입 누적과 첫 방문 독자 확보
- followup: `국제유가 전망에서 다음으로 봐야 할 체크포인트 5가지` / `seo_oil_8` / priority `91.5` / 재방문과 내부링크 순환 강화
- followup: `국제유가 전망 초보자 가이드: 용어부터 시장 반응까지` / `seo_oil_10` / priority `88.5` / 광범위한 초보 검색 수요 흡수
...
- followup: `AI 반도체 주식 관련 대표 종목 한눈에 보기` / `seo_ai_semiconductors_9` / priority `88.5` / 섹터형 검색 유입 누적
- followup: `AI 반도체 주식 공급망 정리: 누가 수혜를 보나` / `seo_ai_semiconductors_11` / priority `85.5` / 체류시간과 페이지뷰 확대
- followup: `AI 반도체 주식 ETF·지수·대표 기업 정리` / `seo_ai_semiconductors_12` / priority `82.5` / 광고 노출과 장기 검색 유입 확보

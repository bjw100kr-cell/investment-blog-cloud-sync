...
- followup: `미국채 금리 상승이 나스닥과 코인에 부담이 되는 이유가 주식과 코인에 미치는 영향` / `seo_treasury_yields_7` / priority `114.5` / 검색 유입 누적과 첫 방문 독자 확보
- followup: `미국채 금리 상승이 나스닥과 코인에 부담이 되는 이유에서 다음으로 봐야 할 체크포인트 5가지` / `seo_treasury_yields_8` / priority `111.5` / 재방문과 내부링크 순환 강화
- followup: `미국채 금리 상승이 나스닥과 코인에 부담이 되는 이유 초보자 가이드: 용어부터 시장 반응까지` / `seo_treasury_yields_9` / priority `108.5` / 광범위한 초보 검색 수요 흡수

## 4. ai_semiconductors 클러스터

- source_keyword: `ai_semiconductors`
- revenue_priority_rank: `99`
- cluster_priority_score: `305.0`
...
- followup: `AI 반도체 주식 관련 대표 종목 한눈에 보기` / `seo_ai_semiconductors_10` / priority `103.5` / 섹터형 검색 유입 누적
- followup: `AI 반도체 주식 ETF·지수·대표 기업 정리` / `seo_ai_semiconductors_12` / priority `97.5` / 광고 노출과 장기 검색 유입 확보

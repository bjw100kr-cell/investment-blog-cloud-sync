...
- followup: `미국채 금리 상승이 나스닥과 코인에 부담이 되는 이유가 주식과 코인에 미치는 영향` / `seo_treasury_yields_4` / priority `123.5` / 검색 유입 누적과 첫 방문 독자 확보
- followup: `미국채 금리 상승이 나스닥과 코인에 부담이 되는 이유에서 다음으로 봐야 할 체크포인트 5가지` / `seo_treasury_yields_6` / priority `120.5` / 재방문과 내부링크 순환 강화
- followup: `미국채 금리 상승이 나스닥과 코인에 부담이 되는 이유 초보자 가이드: 용어부터 시장 반응까지` / `seo_treasury_yields_8` / priority `117.5` / 광범위한 초보 검색 수요 흡수

## 4. 미국 빅테크 허브 클러스터

- source_keyword: `us_big_tech`
- revenue_priority_rank: `99`
- cluster_priority_score: `177.5`
...
- followup: `미국 빅테크 주식 관련 대표 종목 한눈에 보기` / `seo_us_big_tech_11` / priority `88.5` / 섹터형 검색 유입 누적

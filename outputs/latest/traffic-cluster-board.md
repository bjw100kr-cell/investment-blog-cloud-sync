...
- cluster_priority_score: `397.5`
- main_title: `CPI 발표 후 금리와 나스닥, 비트코인이 같이 움직이는 이유`
...
- main_html_path: `/home/runner/work/investment-blog-cloud-sync/investment-blog-cloud-sync/outputs/latest/publish-ready/04-cpi-발표-후-금리와-나스닥-비트코인이-같이-움직이는-이유.html`
- followup: `CPI 발표 후 금리와 나스닥, 비트코인이 같이 움직이는 이유가 주식과 코인에 미치는 영향` / `seo_cpi_9` / priority `101.5` / 검색 유입 누적과 첫 방문 독자 확보
- followup: `CPI 발표 후 금리와 나스닥, 비트코인이 같이 움직이는 이유에서 다음으로 봐야 할 체크포인트 5가지` / `seo_cpi_10` / priority `98.5` / 재방문과 내부링크 순환 강화
- followup: `CPI 발표 후 금리와 나스닥, 비트코인이 같이 움직이는 이유 초보자 가이드: 용어부터 시장 반응까지` / `seo_cpi_11` / priority `95.5` / 광범위한 초보 검색 수요 흡수
...
- followup: `비트코인 현물 ETF 자금 유입: 지금 투자자가 확인할 체크포인트 5가지` / `seo_crypto_etf_8` / priority `107.67` / 새 검색 수요를 빠르게 받아내고 내부링크로 기존 핵심 글과 연결

## 5. us_index_flow 클러스터

- source_keyword: `us_index_flow`
- revenue_priority_rank: `99`
- cluster_priority_score: `95.0`

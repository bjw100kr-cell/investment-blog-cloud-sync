...
- followup: `비트코인 핵심 흐름 초보자 가이드: 지금 꼭 알아야 할 핵심 구조` / `seo_bitcoin_4` / priority `127.5` / 초보 검색 유입과 긴 체류시간 확보
- followup: `비트코인 핵심 흐름 ETF·규제 이슈 정리` / `seo_bitcoin_5` / priority `124.5` / 반복 방문과 뉴스형 검색 유입
- followup: `비트코인 핵심 흐름 FAQ 10개: 많이 헷갈리는 질문 정리` / `seo_bitcoin_6` / priority `121.5` / 롱테일 검색 키워드 확보
...
- main_title: `미국 증시 지수 흐름 해설: 나스닥, 금리, 빅테크 실적을 같이 봐야 하는 이유`
- main_quality_status: `pass`
- main_ready_to_upload: `True`
- followup_count: `3`
- ready_followup_count: `3`
...
- followup: `미국 증시 지수 흐름 관련 대표 종목 한눈에 보기` / `seo_us_index_flow_7` / priority `119.5` / 섹터형 검색 유입 누적
- followup: `미국 증시 지수 흐름 공급망 정리: 누가 수혜를 보나` / `seo_us_index_flow_8` / priority `116.5` / 체류시간과 페이지뷰 확대
- followup: `미국 증시 지수 흐름 ETF·지수·대표 기업 정리` / `seo_us_index_flow_9` / priority `113.5` / 광고 노출과 장기 검색 유입 확보

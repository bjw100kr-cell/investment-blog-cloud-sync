...
예를 들어 `Coinbase debuts tokenized stocks on Base network, joining race to bring equities on blockchain` 같은 제목은 출발점일 뿐입니다. 중요한 건 이 뉴스가 실제로 어떤 자산을 먼저 움직였는지입니다.

## 본문

## 1. 지금 무슨 일이 있었나

이번 글의 출발점은 `Coinbase debuts tokenized stocks on Base network, joining race to bring equities on blockchain`입니다. 다만 제목만 읽고 끝내면 가장 중요한 부분을 놓치기 쉽습니다.

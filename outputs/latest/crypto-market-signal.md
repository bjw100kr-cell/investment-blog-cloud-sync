# Crypto Market Signal

- 생성 시각: `2026-08-04T14:33:54+00:00`
- 상태: `ok`
- 시장 분위기: `extreme_fear`
- 추적 코인 평균 24h 변동률: `1.03%`
- Fear/Greed: `25` (Extreme Fear)

## Tracked Tickers

- `BTCUSDT`: 24h `1.60%`, quote volume `24622140396`
- `ETHUSDT`: 24h `1.00%`, quote volume `6958500721`
- `SOLUSDT`: 24h `1.70%`, quote volume `1423242492`
- `XRPUSDT`: 24h `0.60%`, quote volume `947340912`
- `BNBUSDT`: 24h `0.40%`, quote volume `475980893`
- `DOGEUSDT`: 24h `0.90%`, quote volume `376450771`

## Keyword Signals

- `bitcoin`: bonus `3`, sentiment `extreme_fear`, symbols `BTCUSDT`
  - Fear/Greed 25: Extreme Fear zone
- `ethereum`: bonus `3`, sentiment `extreme_fear`, symbols `ETHUSDT`
  - Fear/Greed 25: Extreme Fear zone
- `crypto_etf`: bonus `6`, sentiment `extreme_fear`, symbols `SOLUSDT, XRPUSDT, BNBUSDT, DOGEUSDT`
  - Fear/Greed 25: Extreme Fear zone

## Editorial Rule

- 이 파일은 코인 글감의 시장 센서입니다. 투자 권유가 아니며, 규제/기업/ETF/기관 관련 문장은 별도 신뢰 소스로 교차 확인합니다.

# Crypto Market Signal

- 생성 시각: `2026-07-10T18:09:11+00:00`
- 상태: `ok`
- 시장 분위기: `extreme_fear`
- 추적 코인 평균 24h 변동률: `0.83%`
- Fear/Greed: `23` (Extreme Fear)

## Tracked Tickers

- `BTCUSDT`: 24h `0.83%`, quote volume `26647215049`
- `ETHUSDT`: 24h `2.18%`, quote volume `8927810304`
- `SOLUSDT`: 24h `-0.55%`, quote volume `1845737427`
- `XRPUSDT`: 24h `0.44%`, quote volume `997153336`
- `BNBUSDT`: 24h `0.76%`, quote volume `500947919`
- `DOGEUSDT`: 24h `1.35%`, quote volume `442119807`

## Keyword Signals

- `bitcoin`: bonus `3`, sentiment `extreme_fear`, symbols `BTCUSDT`
  - Fear/Greed 23: Extreme Fear zone
- `ethereum`: bonus `5`, sentiment `extreme_fear`, symbols `ETHUSDT`
  - ETHUSDT 24h change 2.18%
  - Fear/Greed 23: Extreme Fear zone
- `crypto_etf`: bonus `6`, sentiment `extreme_fear`, symbols `SOLUSDT, XRPUSDT, BNBUSDT, DOGEUSDT`
  - Fear/Greed 23: Extreme Fear zone

## Editorial Rule

- 이 파일은 코인 글감의 시장 센서입니다. 투자 권유가 아니며, 규제/기업/ETF/기관 관련 문장은 별도 신뢰 소스로 교차 확인합니다.

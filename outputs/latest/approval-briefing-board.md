...
- keyword `bitcoin` / publish `2026-08-13` / priority `119.0`
...
- recent_evidence:
  ...
  - Cointelegraph | 2026-08-12T10:06:05+00:00 | Bitcoin miners earn under 0.7% of revenue from fees in new 10-year low
- image_slots:
...
예를 들어 `U.S. CPI inflation slows to 3.4% as expected, bitcoin holds near $64,000` 같은 제목은 출발점일 뿐입니다. 중요한 건 이 뉴스가 실제로 어떤 자산을 먼저 움직였는지입니다.

## 본문

## 1. 지금 무슨 일이 있었나

이번 글의 출발점은 `U.S. CPI inflation slows to 3.4% as expected, bitcoin holds near $64,000`입니다. 다만 제목만 읽고 끝내면 가장 중요한 부분을 놓치기 쉽습니다.
...
- keyword `china` / publish `2026-08-15` / priority `106.0`
- review `approve` score `100` / quality `pass` / ready_now `True`
- intent: 당일 이슈가 내 투자에 어떤 영향을 주는지 빠르게 이해하고 싶은 독자
...
예를 들어 `Meta and Nvidia plant 'very firm flag' in open-weight AI race led by Chinese Labs` 같은 제목은 출발점일 뿐입니다. 중요한 건 이 뉴스가 실제로 어떤 자산을 먼저 움직였는지입니다.

## 본문

## 1. 지금 무슨 일이 있었나

이번 글의 출발점은 `Meta and Nvidia plant 'very firm flag' in open-weight AI race led by Chinese Labs`입니다. 다만 제목만 읽고 끝내면 가장 중요한 부분을 놓치기 쉽습니다.

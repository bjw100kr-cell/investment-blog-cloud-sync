...
- keyword `fomc` / publish `2026-07-03` / priority `137.0`
...
- source_names: CNBC Top News, CoinDesk RSS, Federal Reserve Monetary Policy Press, Financial Times World
- sample_headlines:
...
2026년 7월 3일 기준, 이 이슈를 그냥 뉴스 하나로 넘기기엔 아깝습니다.
...
- 주요 참고 소스: CNBC Top News, CoinDesk RSS, Federal Reserve Monetary Policy Press, Financial Times World
...
- keyword `bitcoin` / publish `2026-07-04` / priority `124.0`
- review `approve` score `100` / quality `pass` / ready_now `True`
- intent: 당일 이슈가 내 투자에 어떤 영향을 주는지 빠르게 이해하고 싶은 독자
...
- recent_evidence:
  ...
  - CoinDesk RSS | 2026-07-03T11:27:44+00:00 | Bitcoin, ether traders aren't fully buying the bounce, options markets show
- image_slots:
...
예를 들어 `Trump says there is ‘nothing wrong’ with family’s crypto windfall` 같은 제목은 출발점일 뿐입니다. 중요한 건 이 뉴스가 실제로 어떤 자산을 먼저 움직였는지입니다.

## 본문

## 1. 지금 무슨 일이 있었나

이번 글의 출발점은 `Trump says there is ‘nothing wrong’ with family’s crypto windfall`입니다. 다만 제목만 읽고 끝내면 가장 중요한 부분을 놓치기 쉽습니다.
...
- keyword `ai_semiconductors` / publish `2026-07-05` / priority `89.0`
- review `approve` score `100` / quality `pass` / ready_now `True`
- intent: 당일 이슈가 내 투자에 어떤 영향을 주는지 빠르게 이해하고 싶은 독자

...
- keyword `fomc` / publish `2026-07-18` / priority `128.0`
...
- keyword `bitcoin` / publish `2026-07-19` / priority `124.0`
- review `approve` score `100` / quality `pass` / ready_now `True`
- intent: 당일 이슈가 내 투자에 어떤 영향을 주는지 빠르게 이해하고 싶은 독자
...
예를 들어 `Stripe and Swift race to control the next generation of global payments infrastructure` 같은 제목은 출발점일 뿐입니다. 중요한 건 이 뉴스가 실제로 어떤 자산을 먼저 움직였는지입니다.

## 본문

## 1. 지금 무슨 일이 있었나

이번 글의 출발점은 `Stripe and Swift race to control the next generation of global payments infrastructure`입니다. 다만 제목만 읽고 끝내면 가장 중요한 부분을 놓치기 쉽습니다.
...
- keyword `ai_semiconductors` / publish `2026-07-20` / priority `121.0`
- review `approve` score `100` / quality `pass` / ready_now `True`
- intent: 당일 이슈가 내 투자에 어떤 영향을 주는지 빠르게 이해하고 싶은 독자
- CTA focus: 실적·공급망·대표 종목 글로 연결
- reason: 복수 소스 교차 확인 가능 (5개), 섹터/세계 흐름 연결 해설 가능
- evidence score: demand `3700` / fallback `source_snapshot_rank` / format `sector_analysis`
...
- recent_evidence:
  ...
  - MarketWatch Breaking News | 2026-07-17T21:55:00+00:00 | The average stock is having a moment as semiconductors struggle. It’s a sign of a healthy market.
- image_slots:
...
예를 들어 `Bitcoin faces fresh headwinds as China’s Kimi beats Claude, GPT in coding benchmark` 같은 제목은 출발점일 뿐입니다. 중요한 건 이 뉴스가 실제로 어떤 자산을 먼저 움직였는지입니다.

## 본문

## 1. 지금 무슨 일이 있었나

이번 글의 출발점은 `Bitcoin faces fresh headwinds as China’s Kimi beats Claude, GPT in coding benchmark`입니다. 다만 제목만 읽고 끝내면 가장 중요한 부분을 놓치기 쉽습니다.

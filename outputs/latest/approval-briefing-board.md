...
- keyword `bitcoin` / publish `2026-08-16` / priority `124.0`
...
- keyword `china` / publish `2026-08-18` / priority `92.0`
- review `approve` score `100` / quality `pass` / ready_now `True`
- intent: 당일 이슈가 내 투자에 어떤 영향을 주는지 빠르게 이해하고 싶은 독자
...
예를 들어 `Beijing is said to move to clarify tax rules stoking confusion among China's ultra-wealthy` 같은 제목은 출발점일 뿐입니다. 중요한 건 이 뉴스가 실제로 어떤 자산을 먼저 움직였는지입니다.

## 본문

## 1. 지금 무슨 일이 있었나

이번 글의 출발점은 `Beijing is said to move to clarify tax rules stoking confusion among China's ultra-wealthy`입니다. 다만 제목만 읽고 끝내면 가장 중요한 부분을 놓치기 쉽습니다.
...
- keyword `us_big_tech` / publish `2026-08-17` / priority `108.0`

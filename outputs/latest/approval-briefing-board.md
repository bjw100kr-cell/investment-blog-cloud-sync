...
- keyword `bitcoin` / publish `2026-08-27` / priority `122.0`
...
- recent_evidence:
  ...
  - Investing.com Crypto News | 2026-08-25 22:11:50 | Bitcoin takes a beat after debasement trade sparks best week in over three years
- image_slots:
...
2026년 8월 26일 기준, 이 이슈를 그냥 뉴스 하나로 넘기기엔 아깝습니다.
...
예를 들어 `Zerohash back for second effort at OCC trust bank charter` 같은 제목은 출발점일 뿐입니다. 중요한 건 이 뉴스가 실제로 어떤 자산을 먼저 움직였는지입니다.

## 본문

## 1. 지금 무슨 일이 있었나

이번 글의 출발점은 `Zerohash back for second effort at OCC trust bank charter`입니다. 다만 제목만 읽고 끝내면 가장 중요한 부분을 놓치기 쉽습니다.
...
- keyword `ai_semiconductors` / publish `2026-08-28` / priority `115.0`
- review `approve` score `100` / quality `pass` / ready_now `True`
- intent: 당일 이슈가 내 투자에 어떤 영향을 주는지 빠르게 이해하고 싶은 독자
- CTA focus: 실적·공급망·대표 종목 글로 연결
- reason: 검색 트렌드 반응 존재, 복수 소스 교차 확인 가능 (4개), 섹터/세계 흐름 연결 해설 가능, 실제 급상승 검색어 반영 (엔비디아 실적)
- evidence score: demand `300` / fallback `trend_match` / format `sector_analysis`
- source_names: Financial Times World, Financial Times YouTube, Google Trends KR, Reuters Markets via Google News RSS
- sample_headlines:
  - 엔비디아 실적
...
- recent_evidence:
  ...
  - Reuters Markets via Google News RSS | 2026-08-25T05:01:00+00:00 | Nvidia shares set for $280 billion price swing after earnings, options show - Reuters
- image_slots:
...
예를 들어 `엔비디아 실적` 같은 제목은 출발점일 뿐입니다. 중요한 건 이 뉴스가 실제로 어떤 자산을 먼저 움직였는지입니다.

## 본문

## 1. 지금 무슨 일이 있었나

이번 글의 출발점은 `엔비디아 실적`입니다. 다만 제목만 읽고 끝내면 가장 중요한 부분을 놓치기 쉽습니다.
...
- 주요 참고 소스: Financial Times World, Financial Times YouTube, Google Trends KR, Reuters Markets via Google News RSS
...
- keyword `china` / publish `2026-08-29` / priority `105.0`
- review `approve` score `100` / quality `pass` / ready_now `True`
- intent: 당일 이슈가 내 투자에 어떤 영향을 주는지 빠르게 이해하고 싶은 독자
- CTA focus: 실적·공급망·대표 종목 글로 연결
- reason: 복수 소스 교차 확인 가능 (4개), 섹터/세계 흐름 연결 해설 가능
- evidence score: demand `2300` / fallback `source_snapshot_rank` / format `macro_explainer`
- source_names: Financial Times Home, Financial Times World, NYT Business, 무역킹 Trade King YouTube
- sample_headlines:
...
- recent_evidence:
  ...
  - 무역킹 Trade King YouTube | 10K views | It's Not About China(?): U.S.-Iran Economic Sanctions Edition
- image_slots:
...
예를 들어 `South Korea challenges China on Arctic trade route` 같은 제목은 출발점일 뿐입니다. 중요한 건 이 뉴스가 실제로 어떤 자산을 먼저 움직였는지입니다.

## 본문

## 1. 지금 무슨 일이 있었나

이번 글의 출발점은 `South Korea challenges China on Arctic trade route`입니다. 다만 제목만 읽고 끝내면 가장 중요한 부분을 놓치기 쉽습니다.
...
- 주요 참고 소스: Financial Times Home, Financial Times World, NYT Business, 무역킹 Trade King YouTube

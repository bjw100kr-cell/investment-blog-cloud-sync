...
- keyword `bitcoin` / publish `2026-09-15` / priority `122.0`
...
예를 들어 `Is Clarity dead? A vibes-based analysis: State of Crypto` 같은 제목은 출발점일 뿐입니다. 중요한 건 이 뉴스가 실제로 어떤 자산을 먼저 움직였는지입니다.

## 본문

## 1. 지금 무슨 일이 있었나

이번 글의 출발점은 `Is Clarity dead? A vibes-based analysis: State of Crypto`입니다. 다만 제목만 읽고 끝내면 가장 중요한 부분을 놓치기 쉽습니다.
...
- keyword `ai_semiconductors` / publish `2026-09-16` / priority `112.0`
- review `approve` score `100` / quality `pass` / ready_now `True`
- intent: 당일 이슈가 내 투자에 어떤 영향을 주는지 빠르게 이해하고 싶은 독자
...
- source_names: CNBC Top News, Cointelegraph, Financial Times Home, Financial Times YouTube
- sample_headlines:
...
예를 들어 `Anthropic chief urges slowdown in AI development to safer pace` 같은 제목은 출발점일 뿐입니다. 중요한 건 이 뉴스가 실제로 어떤 자산을 먼저 움직였는지입니다.

## 본문

## 1. 지금 무슨 일이 있었나

이번 글의 출발점은 `Anthropic chief urges slowdown in AI development to safer pace`입니다. 다만 제목만 읽고 끝내면 가장 중요한 부분을 놓치기 쉽습니다.
...
- 주요 참고 소스: CNBC Top News, Cointelegraph, Financial Times Home, Financial Times YouTube
...
- keyword `china` / publish `2026-09-17` / priority `90.0`
- review `approve` score `100` / quality `pass` / ready_now `True`
- intent: 당일 이슈가 내 투자에 어떤 영향을 주는지 빠르게 이해하고 싶은 독자
- CTA focus: 실적·공급망·대표 종목 글로 연결
- reason: 복수 소스 교차 확인 가능 (3개), 섹터/세계 흐름 연결 해설 가능
- evidence score: demand `0` / fallback `mapped_candidate` / format `macro_explainer`
- source_names: CNBC Top News, Reuters Markets via Google News RSS, 무역킹 Trade King YouTube
- sample_headlines:
  - Anthropic's Amodei says China presents 'toughest dilemma' for his proposed AI slowdown
...
예를 들어 `Anthropic's Amodei says China presents 'toughest dilemma' for his proposed AI slowdown` 같은 제목은 출발점일 뿐입니다. 중요한 건 이 뉴스가 실제로 어떤 자산을 먼저 움직였는지입니다.

## 본문

## 1. 지금 무슨 일이 있었나

이번 글의 출발점은 `Anthropic's Amodei says China presents 'toughest dilemma' for his proposed AI slowdown`입니다. 다만 제목만 읽고 끝내면 가장 중요한 부분을 놓치기 쉽습니다.
...
- 주요 참고 소스: CNBC Top News, Reuters Markets via Google News RSS, 무역킹 Trade King YouTube

...
- keyword `fomc` / publish `2026-07-24` / priority `137.0`
...
- recent_evidence:
  ...
  - Investing.com Crypto News | 2026-07-24 07:06:45 | Bitcoin squeezed near $65.5K: Live breakout levels
- image_slots:
...
예를 들어 `Mirae plans to turn crypto exchange Korbit into something Korea hasn't seen before` 같은 제목은 출발점일 뿐입니다. 중요한 건 이 뉴스가 실제로 어떤 자산을 먼저 움직였는지입니다.

## 본문

## 1. 지금 무슨 일이 있었나

이번 글의 출발점은 `Mirae plans to turn crypto exchange Korbit into something Korea hasn't seen before`입니다. 다만 제목만 읽고 끝내면 가장 중요한 부분을 놓치기 쉽습니다.
...
- keyword `us_big_tech` / publish `2026-07-26` / priority `82.0`
- review `approve` score `100` / quality `pass` / ready_now `True`
- intent: 당일 이슈가 내 투자에 어떤 영향을 주는지 빠르게 이해하고 싶은 독자
...
- source_names: CoinDesk RSS, MarketWatch Breaking News
- sample_headlines:
  - Bitcoin holds near $65,000 as $800 billion AI selloff leaves crypto largely untouched
...
예를 들어 `Bitcoin holds near $65,000 as $800 billion AI selloff leaves crypto largely untouched` 같은 제목은 출발점일 뿐입니다. 중요한 건 이 뉴스가 실제로 어떤 자산을 먼저 움직였는지입니다.

## 본문

## 1. 지금 무슨 일이 있었나

이번 글의 출발점은 `Bitcoin holds near $65,000 as $800 billion AI selloff leaves crypto largely untouched`입니다. 다만 제목만 읽고 끝내면 가장 중요한 부분을 놓치기 쉽습니다.
...
- 주요 참고 소스: CoinDesk RSS, MarketWatch Breaking News

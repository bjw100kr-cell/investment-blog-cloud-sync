...
- keyword `fomc` / publish `2026-07-01` / priority `137.0`
...
- source_names: CoinDesk RSS, Federal Reserve Monetary Policy Press, Financial Times Home, MarketWatch Breaking News, NYT Business
- sample_headlines:
...
- 주요 참고 소스: CoinDesk RSS, Federal Reserve Monetary Policy Press, Financial Times Home, MarketWatch Breaking News, NYT Business
...
- recent_evidence:
  ...
  - CoinDesk RSS | 2026-07-01T15:11:46+00:00 | Cantor says bitcoin bear market may be entering final stretch
- image_slots:
...
예를 들어 `Robinhood rolls out public blockchain as it expands deeper into crypto` 같은 제목은 출발점일 뿐입니다. 중요한 건 이 뉴스가 실제로 어떤 자산을 먼저 움직였는지입니다.

## 본문

## 1. 지금 무슨 일이 있었나

이번 글의 출발점은 `Robinhood rolls out public blockchain as it expands deeper into crypto`입니다. 다만 제목만 읽고 끝내면 가장 중요한 부분을 놓치기 쉽습니다.
...
- keyword `ai_semiconductors` / publish `2026-07-03` / priority `93.0`
- review `approve` score `100` / quality `pass` / ready_now `True`
- intent: 당일 이슈가 내 투자에 어떤 영향을 주는지 빠르게 이해하고 싶은 독자
...
- source_names: CNBC Top News, NYT Business
- sample_headlines:
...
예를 들어 `Trump says outside funds 'run my money' after disclosure shows billions in 2025 revenue` 같은 제목은 출발점일 뿐입니다. 중요한 건 이 뉴스가 실제로 어떤 자산을 먼저 움직였는지입니다.

## 본문

## 1. 지금 무슨 일이 있었나

이번 글의 출발점은 `Trump says outside funds 'run my money' after disclosure shows billions in 2025 revenue`입니다. 다만 제목만 읽고 끝내면 가장 중요한 부분을 놓치기 쉽습니다.
...
- 주요 참고 소스: CNBC Top News, NYT Business

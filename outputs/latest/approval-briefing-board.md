...
- keyword `fomc` / publish `2026-08-20` / priority `137.0`
...
- evidence score: demand `4300` / fallback `source_snapshot_rank` / format `macro_explainer`
- source_names: CNBC Top News, Federal Reserve Monetary Policy Press, Financial Times Home, Financial Times World, NYT Business
- sample_headlines:
...
2026년 8월 20일 기준, 이 이슈를 그냥 뉴스 하나로 넘기기엔 아깝습니다.
...
- 주요 참고 소스: CNBC Top News, Federal Reserve Monetary Policy Press, Financial Times Home, Financial Times World, NYT Business
...
- keyword `bitcoin` / publish `2026-08-21` / priority `126.0`
- review `approve` score `100` / quality `pass` / ready_now `True`
- intent: 당일 이슈가 내 투자에 어떤 영향을 주는지 빠르게 이해하고 싶은 독자
...
- recent_evidence:
  ...
  - CoinDesk RSS | 2026-08-19T15:22:47+00:00 | Bitcoin nears key technical breakout that could propel prices to $76,000
- image_slots:
...
예를 들어 `Bitcoin briefly hits $70,000 for the first time since June. Here is why` 같은 제목은 출발점일 뿐입니다. 중요한 건 이 뉴스가 실제로 어떤 자산을 먼저 움직였는지입니다.

## 본문

## 1. 지금 무슨 일이 있었나

이번 글의 출발점은 `Bitcoin briefly hits $70,000 for the first time since June. Here is why`입니다. 다만 제목만 읽고 끝내면 가장 중요한 부분을 놓치기 쉽습니다.
...
- keyword `ai_semiconductors` / publish `2026-08-22` / priority `109.0`
- review `approve` score `100` / quality `pass` / ready_now `True`
- intent: 당일 이슈가 내 투자에 어떤 영향을 주는지 빠르게 이해하고 싶은 독자
...
- recent_evidence:
  ...
  - MarketWatch Breaking News | 2026-08-19T22:02:00+00:00 | The chip-stock rally hits a speed bump — but these analysts see reason to be hopeful
- image_slots:
...
예를 들어 `Japan exports beat estimates in July as robust chip shipments power fifth straight month of gains` 같은 제목은 출발점일 뿐입니다. 중요한 건 이 뉴스가 실제로 어떤 자산을 먼저 움직였는지입니다.

## 본문

## 1. 지금 무슨 일이 있었나

이번 글의 출발점은 `Japan exports beat estimates in July as robust chip shipments power fifth straight month of gains`입니다. 다만 제목만 읽고 끝내면 가장 중요한 부분을 놓치기 쉽습니다.

...
- keyword `bitcoin` / publish `2026-09-04` / priority `122.0`
...
- recent_evidence:
  ...
  - Investing.com Crypto News | 2026-09-02 19:19:20 | Bitcoin trapped at $77K support/resistance: Live levels
- image_slots:
...
예를 들어 `DOJ says Hamas crypto seizures reached $560,000 as FBI took over fundraising sites` 같은 제목은 출발점일 뿐입니다. 중요한 건 이 뉴스가 실제로 어떤 자산을 먼저 움직였는지입니다.

## 본문

## 1. 지금 무슨 일이 있었나

이번 글의 출발점은 `DOJ says Hamas crypto seizures reached $560,000 as FBI took over fundraising sites`입니다. 다만 제목만 읽고 끝내면 가장 중요한 부분을 놓치기 쉽습니다.
...
- keyword `us_big_tech` / publish `2026-09-05` / priority `102.0`
- review `approve` score `100` / quality `pass` / ready_now `True`
- intent: 당일 이슈가 내 투자에 어떤 영향을 주는지 빠르게 이해하고 싶은 독자
...
예를 들어 `Microsoft to start disclosing Azure quarterly revenue as company consolidates business units` 같은 제목은 출발점일 뿐입니다. 중요한 건 이 뉴스가 실제로 어떤 자산을 먼저 움직였는지입니다.

## 본문

## 1. 지금 무슨 일이 있었나

이번 글의 출발점은 `Microsoft to start disclosing Azure quarterly revenue as company consolidates business units`입니다. 다만 제목만 읽고 끝내면 가장 중요한 부분을 놓치기 쉽습니다.
...
- keyword `china` / publish `2026-09-06` / priority `85.0`
- review `approve` score `100` / quality `pass` / ready_now `True`
- intent: 당일 이슈가 내 투자에 어떤 영향을 주는지 빠르게 이해하고 싶은 독자
- CTA focus: 실적·공급망·대표 종목 글로 연결
- reason: 검색 트렌드 반응 존재, 섹터/세계 흐름 연결 해설 가능, 실제 급상승 검색어 반영 (중국공산당)
- evidence score: demand `700` / fallback `trend_match` / format `macro_explainer`
...
쉽게 말해 실시간 검색 흐름를 같이 보면 headline 뒤에 있는 자금 흐름과 심리 변화를 더 빨리 잡을 수 있습니다.
예를 들어 `중국공산당` 같은 제목은 출발점일 뿐입니다. 중요한 건 이 뉴스가 실제로 어떤 자산을 먼저 움직였는지입니다.

## 본문

## 1. 지금 무슨 일이 있었나

이번 글의 출발점은 `중국공산당`입니다. 다만 제목만 읽고 끝내면 가장 중요한 부분을 놓치기 쉽습니다.

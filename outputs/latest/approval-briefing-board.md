...
- batch confirmation: `python3 /home/runner/work/investment-blog-cloud-sync/investment-blog-cloud-sync/scripts/set_review_approvals.py --keywords bitcoin treasury_yields`

## 1. 미국채 금리 상승이 나스닥과 코인에 부담이 되는 이유

- keyword `treasury_yields` / publish `2026-08-08` / priority `112.0`
...
- evidence score: demand `2600` / fallback `source_snapshot_rank` / format `macro_explainer`
- source_names: CoinDesk RSS, Cointelegraph
- sample_headlines:
...
2026년 8월 8일 기준, 이 이슈를 그냥 뉴스 하나로 넘기기엔 아깝습니다.
...
예를 들어 `Trump Media pulls back from crypto, scraps Crypto.com's CRO token treasury deal` 같은 제목은 출발점일 뿐입니다. 중요한 건 이 뉴스가 실제로 어떤 자산을 먼저 움직였는지입니다.

## 본문

## 1. 지금 무슨 일이 있었나

이번 글의 출발점은 `Trump Media pulls back from crypto, scraps Crypto.com's CRO token treasury deal`입니다. 다만 제목만 읽고 끝내면 가장 중요한 부분을 놓치기 쉽습니다.
...
- 주요 참고 소스: CoinDesk RSS, Cointelegraph
...
- keyword `bitcoin` / publish `2026-08-09` / priority `126.0`
- review `approve` score `100` / quality `pass` / ready_now `True`
- intent: 당일 이슈가 내 투자에 어떤 영향을 주는지 빠르게 이해하고 싶은 독자
- CTA focus: ETF·규제·초보 가이드 글로 연결
- reason: 복수 소스 교차 확인 가능 (5개), 코인 독자 유입과 재방문 가능성, 코인 시장 신호 반영 (mixed)
- evidence score: demand `7900` / fallback `source_snapshot_rank` / format `crypto_analysis`
- source_names: CoinDesk RSS, Cointelegraph, Financial Times Home, Financial Times World, Investing.com Crypto News
- sample_headlines:
...
- recent_evidence:
  ...
  - Investing.com Crypto News | 2026-08-08 07:02:11 | Bitcoin stalls at $65,156 resistance as ADX warns of bull trap: Live levels
- image_slots:
...
예를 들어 `Why trillion-dollar asset manager T. Rowe Price put memecoins in its crypto ETF` 같은 제목은 출발점일 뿐입니다. 중요한 건 이 뉴스가 실제로 어떤 자산을 먼저 움직였는지입니다.

## 본문

## 1. 지금 무슨 일이 있었나

이번 글의 출발점은 `Why trillion-dollar asset manager T. Rowe Price put memecoins in its crypto ETF`입니다. 다만 제목만 읽고 끝내면 가장 중요한 부분을 놓치기 쉽습니다.
...
- 주요 참고 소스: CoinDesk RSS, Cointelegraph, Financial Times Home, Financial Times World, Investing.com Crypto News
...
- keyword `us_index_flow` / publish `2026-08-10` / priority `101.0`
- review `approve` score `100` / quality `pass` / ready_now `True`
- intent: 당일 이슈가 내 투자에 어떤 영향을 주는지 빠르게 이해하고 싶은 독자
...
- source_names: CNBC Top News, Financial Times Home, Reuters Markets via Google News RSS
- sample_headlines:
...
예를 들어 `A record-breaking week for options powers S&P 500 surge; volatility gauge is near 2026 low` 같은 제목은 출발점일 뿐입니다. 중요한 건 이 뉴스가 실제로 어떤 자산을 먼저 움직였는지입니다.

## 본문

## 1. 지금 무슨 일이 있었나

이번 글의 출발점은 `A record-breaking week for options powers S&P 500 surge; volatility gauge is near 2026 low`입니다. 다만 제목만 읽고 끝내면 가장 중요한 부분을 놓치기 쉽습니다.

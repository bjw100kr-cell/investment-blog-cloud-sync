...
- keyword `bitcoin` / publish `2026-08-20` / priority `122.0`
...
- recent_evidence:
  ...
  - Cointelegraph | 2026-08-19T11:06:13+00:00 | Bitcoin has ‘largely purged’ froth that preceded 50% drop from $126K: BlackRock
- image_slots:
...
예를 들어 `Bitcoin is flashing 8 of 12 capitulation signals, but bottom's not yet in, says VanEck` 같은 제목은 출발점일 뿐입니다. 중요한 건 이 뉴스가 실제로 어떤 자산을 먼저 움직였는지입니다.

## 본문

## 1. 지금 무슨 일이 있었나

이번 글의 출발점은 `Bitcoin is flashing 8 of 12 capitulation signals, but bottom's not yet in, says VanEck`입니다. 다만 제목만 읽고 끝내면 가장 중요한 부분을 놓치기 쉽습니다.
...
- keyword `ai_semiconductors` / publish `2026-08-21` / priority `90.0`
- review `approve` score `100` / quality `pass` / ready_now `True`
- intent: 당일 이슈가 내 투자에 어떤 영향을 주는지 빠르게 이해하고 싶은 독자
...
예를 들어 `OpenAI trails Anthropic as losses deepen and Altman pauses frontier AI training` 같은 제목은 출발점일 뿐입니다. 중요한 건 이 뉴스가 실제로 어떤 자산을 먼저 움직였는지입니다.

## 본문

## 1. 지금 무슨 일이 있었나

이번 글의 출발점은 `OpenAI trails Anthropic as losses deepen and Altman pauses frontier AI training`입니다. 다만 제목만 읽고 끝내면 가장 중요한 부분을 놓치기 쉽습니다.
...
- keyword `china` / publish `2026-08-22` / priority `90.0`
- review `approve` score `100` / quality `pass` / ready_now `True`
- intent: 당일 이슈가 내 투자에 어떤 영향을 주는지 빠르게 이해하고 싶은 독자
- CTA focus: 실적·공급망·대표 종목 글로 연결
- reason: 복수 소스 교차 확인 가능 (3개), 섹터/세계 흐름 연결 해설 가능
- evidence score: demand `0` / fallback `mapped_candidate` / format `macro_explainer`
- source_names: CoinDesk RSS, MarketWatch Breaking News, NYT Business
- sample_headlines:
...
예를 들어 `China triples its e-CNY network in 2026 as 8 more banks join the CBDC push this week` 같은 제목은 출발점일 뿐입니다. 중요한 건 이 뉴스가 실제로 어떤 자산을 먼저 움직였는지입니다.

## 본문

## 1. 지금 무슨 일이 있었나

이번 글의 출발점은 `China triples its e-CNY network in 2026 as 8 more banks join the CBDC push this week`입니다. 다만 제목만 읽고 끝내면 가장 중요한 부분을 놓치기 쉽습니다.
...
- 주요 참고 소스: CoinDesk RSS, MarketWatch Breaking News, NYT Business

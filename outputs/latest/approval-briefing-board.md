...
- keyword `bitcoin` / publish `2026-09-17` / priority `128.0`
- review `approve` score `100` / quality `pass` / ready_now `True`
- intent: 당일 이슈가 내 투자에 어떤 영향을 주는지 빠르게 이해하고 싶은 독자
- CTA focus: ETF·규제·초보 가이드 글로 연결
- reason: 복수 소스 교차 확인 가능 (6개), 코인 독자 유입과 재방문 가능성, 코인 시장 신호 반영 (risk_off)
- evidence score: demand `7500` / fallback `source_snapshot_rank` / format `crypto_analysis`
- source_names: CoinDesk RSS, Cointelegraph, Investing.com Crypto News, MarketWatch Breaking News, NYT Business, Reuters Markets via Google News RSS
- sample_headlines:
...
- recent_evidence:
  ...
  - Reuters Markets via Google News RSS | 2026-09-15T19:45:43+00:00 | Bitcoin and crypto stocks remain down after US senate fails to advance regulatory bill - Reuters
- image_slots:
...
2026년 9월 16일 기준, 이 이슈를 그냥 뉴스 하나로 넘기기엔 아깝습니다.
...
예를 들어 `Crypto longs worth $570 million wiped out as Clarity Act fails` 같은 제목은 출발점일 뿐입니다. 중요한 건 이 뉴스가 실제로 어떤 자산을 먼저 움직였는지입니다.

## 본문

## 1. 지금 무슨 일이 있었나

이번 글의 출발점은 `Crypto longs worth $570 million wiped out as Clarity Act fails`입니다. 다만 제목만 읽고 끝내면 가장 중요한 부분을 놓치기 쉽습니다.
...
- 주요 참고 소스: CoinDesk RSS, Cointelegraph, Investing.com Crypto News, MarketWatch Breaking News, NYT Business, Reuters Markets via Google News RSS
...
- keyword `china` / publish `2026-09-19` / priority `111.0`
- review `approve` score `100` / quality `pass` / ready_now `True`
- intent: 당일 이슈가 내 투자에 어떤 영향을 주는지 빠르게 이해하고 싶은 독자
- CTA focus: 실적·공급망·대표 종목 글로 연결
- reason: 복수 소스 교차 확인 가능 (3개), 섹터/세계 흐름 연결 해설 가능
- evidence score: demand `3000` / fallback `source_snapshot_rank` / format `macro_explainer`
...
예를 들어 `China's AI leaders keep quiet despite U.S. 'publicity' on tech risks` 같은 제목은 출발점일 뿐입니다. 중요한 건 이 뉴스가 실제로 어떤 자산을 먼저 움직였는지입니다.

## 본문

## 1. 지금 무슨 일이 있었나

이번 글의 출발점은 `China's AI leaders keep quiet despite U.S. 'publicity' on tech risks`입니다. 다만 제목만 읽고 끝내면 가장 중요한 부분을 놓치기 쉽습니다.
...
- 주요 참고 소스: CNBC Top News, Financial Times Home, Financial Times World
...
- keyword `ai_semiconductors` / publish `2026-09-18` / priority `105.0`
- review `approve` score `100` / quality `pass` / ready_now `True`
- intent: 당일 이슈가 내 투자에 어떤 영향을 주는지 빠르게 이해하고 싶은 독자
- CTA focus: 실적·공급망·대표 종목 글로 연결
- reason: 복수 소스 교차 확인 가능 (4개), 섹터/세계 흐름 연결 해설 가능
- evidence score: demand `0` / fallback `mapped_candidate` / format `sector_analysis`
- source_names: CNBC Top News, Financial Times Home, Financial Times YouTube, MarketWatch Breaking News
- sample_headlines:
...
예를 들어 `Nvidia's Huang diverges with CEOs of Anthropic, OpenAI on AI safety at Dreamforce` 같은 제목은 출발점일 뿐입니다. 중요한 건 이 뉴스가 실제로 어떤 자산을 먼저 움직였는지입니다.

## 본문

## 1. 지금 무슨 일이 있었나

이번 글의 출발점은 `Nvidia's Huang diverges with CEOs of Anthropic, OpenAI on AI safety at Dreamforce`입니다. 다만 제목만 읽고 끝내면 가장 중요한 부분을 놓치기 쉽습니다.
...
- 주요 참고 소스: CNBC Top News, Financial Times Home, Financial Times YouTube, MarketWatch Breaking News

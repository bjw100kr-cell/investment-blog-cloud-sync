...
- keyword `bitcoin` / publish `2026-07-08` / priority `123.0`
...
- recent_evidence:
  ...
  - CoinDesk RSS | 2026-07-07T08:20:00+00:00 | Binance taps into Bitcoin holders’ hunger for yield with new covered call yield play
- image_slots:
...
예를 들어 `Bitcoin stalls as open interest decline raises questions about rally's staying power` 같은 제목은 출발점일 뿐입니다. 중요한 건 이 뉴스가 실제로 어떤 자산을 먼저 움직였는지입니다.

## 본문

## 1. 지금 무슨 일이 있었나

이번 글의 출발점은 `Bitcoin stalls as open interest decline raises questions about rally's staying power`입니다. 다만 제목만 읽고 끝내면 가장 중요한 부분을 놓치기 쉽습니다.
...
- keyword `china` / publish `2026-07-10` / priority `74.0`
...
예를 들어 `China breaks step with global markets, and investors buy in - Reuters` 같은 제목은 출발점일 뿐입니다. 중요한 건 이 뉴스가 실제로 어떤 자산을 먼저 움직였는지입니다.

## 본문

## 1. 지금 무슨 일이 있었나

이번 글의 출발점은 `China breaks step with global markets, and investors buy in - Reuters`입니다. 다만 제목만 읽고 끝내면 가장 중요한 부분을 놓치기 쉽습니다.
...
- keyword `ai_growth_stocks` / publish `2026-07-09` / priority `71.0`
- review `approve` score `100` / quality `review_before_publish` / ready_now `False`
- intent: 당일 이슈가 내 투자에 어떤 영향을 주는지 빠르게 이해하고 싶은 독자
- CTA focus: 실적·공급망·대표 종목 글로 연결
...
예를 들어 `Palantir Is Moving the Japanese Military` 같은 제목은 출발점일 뿐입니다. 중요한 건 이 뉴스가 실제로 어떤 자산을 먼저 움직였는지입니다.

## 본문

## 1. 지금 무슨 일이 있었나

이번 글의 출발점은 `Palantir Is Moving the Japanese Military`입니다. 다만 제목만 읽고 끝내면 가장 중요한 부분을 놓치기 쉽습니다.

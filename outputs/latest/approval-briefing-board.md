...
- keyword `bitcoin` / publish `2026-07-06` / priority `126.0`
...
- recent_evidence:
  ...
  - CoinDesk RSS | 2026-07-04T18:00:00+00:00 | Bitcoin experts split over plan to freeze Satoshi's 1.1 million bitcoin as quantum threat grows
- image_slots:
...
- keyword `ai_semiconductors` / publish `2026-07-07` / priority `120.0`
...
- evidence score: demand `20200` / fallback `trend_match` / format `sector_analysis`
...
예를 들어 `반도체` 같은 제목은 출발점일 뿐입니다. 중요한 건 이 뉴스가 실제로 어떤 자산을 먼저 움직였는지입니다.

## 본문

## 1. 지금 무슨 일이 있었나

이번 글의 출발점은 `반도체`입니다. 다만 제목만 읽고 끝내면 가장 중요한 부분을 놓치기 쉽습니다.
...
- 주요 참고 소스: Google Trends KR, NYT Business, Reuters Markets via Google News RSS
...
- keyword `china` / publish `2026-07-08` / priority `80.0`
- review `approve` score `100` / quality `review_before_publish` / ready_now `False`
- intent: 당일 이슈가 내 투자에 어떤 영향을 주는지 빠르게 이해하고 싶은 독자
- CTA focus: 실적·공급망·대표 종목 글로 연결
...
예를 들어 `Wall Street banks recover in China amid trading boom` 같은 제목은 출발점일 뿐입니다. 중요한 건 이 뉴스가 실제로 어떤 자산을 먼저 움직였는지입니다.

## 본문

## 1. 지금 무슨 일이 있었나

이번 글의 출발점은 `Wall Street banks recover in China amid trading boom`입니다. 다만 제목만 읽고 끝내면 가장 중요한 부분을 놓치기 쉽습니다.

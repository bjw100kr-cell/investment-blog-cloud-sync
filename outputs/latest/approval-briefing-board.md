...
- keyword `bitcoin` / publish `2026-07-20` / priority `124.0`
...
- recent_evidence:
  ...
  - Investing.com Crypto News | 2026-07-19 08:57:01 | Bitcoin eyes $72,000 as governance and mining concentration concerns grow
- image_slots:
...
2026년 7월 19일 기준, 이 이슈를 그냥 뉴스 하나로 넘기기엔 아깝습니다.
...
예를 들어 `Bitcoin's biggest advocate, Michael Saylor, says new plan to clean up the blockchain is 'a bad idea'` 같은 제목은 출발점일 뿐입니다. 중요한 건 이 뉴스가 실제로 어떤 자산을 먼저 움직였는지입니다.

## 본문

## 1. 지금 무슨 일이 있었나

이번 글의 출발점은 `Bitcoin's biggest advocate, Michael Saylor, says new plan to clean up the blockchain is 'a bad idea'`입니다. 다만 제목만 읽고 끝내면 가장 중요한 부분을 놓치기 쉽습니다.
...
- keyword `ai_semiconductors` / publish `2026-07-21` / priority `110.0`
- review `approve` score `100` / quality `pass` / ready_now `True`
- intent: 당일 이슈가 내 투자에 어떤 영향을 주는지 빠르게 이해하고 싶은 독자
...
- source_names: Financial Times YouTube, MarketWatch Breaking News, Reuters Markets via Google News RSS
- sample_headlines:
...
- recent_evidence:
  ...
  - Reuters Markets via Google News RSS | 2026-07-17T20:55:29+00:00 | Chip stock pullback sparks worries about AI rally strength, leveraged trades - Reuters
- image_slots:
...
예를 들어 `Chip stock pullback sparks worries about AI rally strength, leveraged trades - Reuters` 같은 제목은 출발점일 뿐입니다. 중요한 건 이 뉴스가 실제로 어떤 자산을 먼저 움직였는지입니다.

## 본문

## 1. 지금 무슨 일이 있었나

이번 글의 출발점은 `Chip stock pullback sparks worries about AI rally strength, leveraged trades - Reuters`입니다. 다만 제목만 읽고 끝내면 가장 중요한 부분을 놓치기 쉽습니다.
...
- 주요 참고 소스: Financial Times YouTube, MarketWatch Breaking News, Reuters Markets via Google News RSS
...
- keyword `china` / publish `2026-07-22` / priority `80.0`
- review `approve` score `100` / quality `pass` / ready_now `True`
- intent: 당일 이슈가 내 투자에 어떤 영향을 주는지 빠르게 이해하고 싶은 독자
- CTA focus: 실적·공급망·대표 종목 글로 연결
- reason: 복수 소스 교차 확인 가능 (2개), 섹터/세계 흐름 연결 해설 가능
- evidence score: demand `0` / fallback `mapped_candidate` / format `macro_explainer`
- source_names: Financial Times World, Reuters Markets via Google News RSS
- sample_headlines:
...
예를 들어 `China's Jingye Steel urges UK to compensate its investment losses - Reuters` 같은 제목은 출발점일 뿐입니다. 중요한 건 이 뉴스가 실제로 어떤 자산을 먼저 움직였는지입니다.

## 본문

## 1. 지금 무슨 일이 있었나

이번 글의 출발점은 `China's Jingye Steel urges UK to compensate its investment losses - Reuters`입니다. 다만 제목만 읽고 끝내면 가장 중요한 부분을 놓치기 쉽습니다.
...
- 주요 참고 소스: Financial Times World, Reuters Markets via Google News RSS

...
- keyword `fomc` / publish `2026-08-27` / priority `137.0`
...
- keyword `bitcoin` / publish `2026-08-28` / priority `122.0`
- review `approve` score `100` / quality `pass` / ready_now `True`
- intent: 당일 이슈가 내 투자에 어떤 영향을 주는지 빠르게 이해하고 싶은 독자
...
- recent_evidence:
  ...
  - Investing.com Crypto News | 2026-08-27 19:19:51 | Bitcoin consolidates at $80K as MACD turns bearish: Live levels
- image_slots:
...
예를 들어 `Polish Olympic chief arrested as prosecutors probe suspected crypto-linked bribe` 같은 제목은 출발점일 뿐입니다. 중요한 건 이 뉴스가 실제로 어떤 자산을 먼저 움직였는지입니다.

## 본문

## 1. 지금 무슨 일이 있었나

이번 글의 출발점은 `Polish Olympic chief arrested as prosecutors probe suspected crypto-linked bribe`입니다. 다만 제목만 읽고 끝내면 가장 중요한 부분을 놓치기 쉽습니다.
...
- keyword `ai_semiconductors` / publish `2026-08-29` / priority `118.0`
- review `approve` score `100` / quality `pass` / ready_now `True`
- intent: 당일 이슈가 내 투자에 어떤 영향을 주는지 빠르게 이해하고 싶은 독자
...
- source_names: CNBC Top News, Cointelegraph, Financial Times World, Financial Times YouTube, MarketWatch Breaking News, Reuters Markets via Google News RSS
- sample_headlines:
...
예를 들어 `Bitcoin eyes $81K as Nvidia earnings beat boosts risk assets` 같은 제목은 출발점일 뿐입니다. 중요한 건 이 뉴스가 실제로 어떤 자산을 먼저 움직였는지입니다.

## 본문

## 1. 지금 무슨 일이 있었나

이번 글의 출발점은 `Bitcoin eyes $81K as Nvidia earnings beat boosts risk assets`입니다. 다만 제목만 읽고 끝내면 가장 중요한 부분을 놓치기 쉽습니다.
...
- 주요 참고 소스: CNBC Top News, Cointelegraph, Financial Times World, Financial Times YouTube, MarketWatch Breaking News, Reuters Markets via Google News RSS

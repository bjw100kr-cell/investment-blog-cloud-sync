...
- keyword `bitcoin` / publish `2026-07-09` / priority `124.0`
- review `approve` score `100` / quality `pass` / ready_now `True`
- intent: 당일 이슈가 내 투자에 어떤 영향을 주는지 빠르게 이해하고 싶은 독자
- CTA focus: ETF·규제·초보 가이드 글로 연결
- reason: 검색 트렌드 반응 존재, 복수 소스 교차 확인 가능 (4개), 코인 독자 유입과 재방문 가능성, 코인 시장 신호 반영 (extreme_fear), 실제 급상승 검색어 반영 (bitcoin)
- evidence score: demand `1200` / fallback `trend_match` / format `crypto_analysis`
...
- recent_evidence:
  ...
  - Cointelegraph | 2026-07-08T02:40:13+00:00 | Strike launches ‘volatility-proof’ Bitcoin loans amid bear market, but at a cost
- image_slots:
...
예를 들어 `bitcoin` 같은 제목은 출발점일 뿐입니다. 중요한 건 이 뉴스가 실제로 어떤 자산을 먼저 움직였는지입니다.

## 본문

## 1. 지금 무슨 일이 있었나

이번 글의 출발점은 `bitcoin`입니다. 다만 제목만 읽고 끝내면 가장 중요한 부분을 놓치기 쉽습니다.
...
- keyword `us_index_flow` / publish `2026-07-10` / priority `119.0`
- review `approve` score `100` / quality `pass` / ready_now `True`
- intent: 당일 이슈가 내 투자에 어떤 영향을 주는지 빠르게 이해하고 싶은 독자
- CTA focus: 실적·공급망·대표 종목 글로 연결
- reason: 검색 트렌드 반응 존재, 복수 소스 교차 확인 가능 (3개), 검색량 높은 미국 증시 키워드를 시장 맥락으로 해설 가능, 실제 급상승 검색어 반영 (stock market today)
- evidence score: demand `1200` / fallback `trend_match` / format `sector_analysis`
- source_names: Google Trends US, MarketWatch Breaking News, Reuters Markets via Google News RSS
- sample_headlines:
  - stock market today
...
- recent_evidence:
  ...
  - Reuters Markets via Google News RSS | 2026-07-07T22:19:34+00:00 | Nasdaq sinks as AI worries hit chipmakers - Reuters
- image_slots:
...
예를 들어 `stock market today` 같은 제목은 출발점일 뿐입니다. 중요한 건 이 뉴스가 실제로 어떤 자산을 먼저 움직였는지입니다.

## 본문

## 1. 지금 무슨 일이 있었나

이번 글의 출발점은 `stock market today`입니다. 다만 제목만 읽고 끝내면 가장 중요한 부분을 놓치기 쉽습니다.
...
- 주요 참고 소스: Google Trends US, MarketWatch Breaking News, Reuters Markets via Google News RSS
...
- keyword `china` / publish `2026-07-11` / priority `85.0`
- review `approve` score `100` / quality `pass` / ready_now `True`
- intent: 당일 이슈가 내 투자에 어떤 영향을 주는지 빠르게 이해하고 싶은 독자
- CTA focus: 실적·공급망·대표 종목 글로 연결
...
예를 들어 `China warns about AI risks with Anthropic's Claude Code` 같은 제목은 출발점일 뿐입니다. 중요한 건 이 뉴스가 실제로 어떤 자산을 먼저 움직였는지입니다.

## 본문

## 1. 지금 무슨 일이 있었나

이번 글의 출발점은 `China warns about AI risks with Anthropic's Claude Code`입니다. 다만 제목만 읽고 끝내면 가장 중요한 부분을 놓치기 쉽습니다.

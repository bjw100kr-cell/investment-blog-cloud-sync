...
- keyword `fomc` / publish `2026-07-08` / priority `137.0`
...
2026년 7월 8일 기준, 이 이슈를 그냥 뉴스 하나로 넘기기엔 아깝습니다.
...
- keyword `bitcoin` / publish `2026-07-09` / priority `126.0`
- review `approve` score `100` / quality `pass` / ready_now `True`
- intent: 당일 이슈가 내 투자에 어떤 영향을 주는지 빠르게 이해하고 싶은 독자
- CTA focus: ETF·규제·초보 가이드 글로 연결
- reason: 복수 소스 교차 확인 가능 (3개), 코인 독자 유입과 재방문 가능성, 코인 시장 신호 반영 (extreme_fear)
- evidence score: demand `7000` / fallback `source_snapshot_rank` / format `crypto_analysis`
...
- recent_evidence:
  ...
  - CoinDesk RSS | 2026-07-08T12:44:27+00:00 | SpaceX's first bitcoin wallet movements in six months likely don't signal sales
- image_slots:
...
예를 들어 `Crypto Long & Short: With MSTR concerns assuaged, look to traditional signals around BTC` 같은 제목은 출발점일 뿐입니다. 중요한 건 이 뉴스가 실제로 어떤 자산을 먼저 움직였는지입니다.

## 본문

## 1. 지금 무슨 일이 있었나

이번 글의 출발점은 `Crypto Long & Short: With MSTR concerns assuaged, look to traditional signals around BTC`입니다. 다만 제목만 읽고 끝내면 가장 중요한 부분을 놓치기 쉽습니다.
...
- keyword `ai_growth_stocks` / publish `2026-07-10` / priority `97.0`
...
- evidence score: demand `2000` / fallback `source_snapshot_rank` / format `sector_analysis`
- source_names: Financial Times Home, Financial Times YouTube, 무역킹 Trade King YouTube
- sample_headlines:
...
예를 들어 `Palantir’s real weak spot` 같은 제목은 출발점일 뿐입니다. 중요한 건 이 뉴스가 실제로 어떤 자산을 먼저 움직였는지입니다.

## 본문

## 1. 지금 무슨 일이 있었나

이번 글의 출발점은 `Palantir’s real weak spot`입니다. 다만 제목만 읽고 끝내면 가장 중요한 부분을 놓치기 쉽습니다.
...
- 주요 참고 소스: Financial Times Home, Financial Times YouTube, 무역킹 Trade King YouTube

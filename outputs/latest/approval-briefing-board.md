...
- keyword `bitcoin` / publish `2026-09-16` / priority `126.0`
...
- evidence score: demand `8000` / fallback `source_snapshot_rank` / format `crypto_analysis`
- source_names: CoinDesk RSS, Cointelegraph, Financial Times Home, Financial Times World, Investing.com Crypto News, MarketWatch Breaking News
- sample_headlines:
...
- recent_evidence:
  ...
  - Investing.com Crypto News | 2026-09-14 21:56:58 | Bitcoin up 2% after Trump agrees to ethics requirements in latest Clarity Act text
- image_slots:
...
예를 들어 `U.S. House panel shares crypto tax bill ahead of hearing later this week` 같은 제목은 출발점일 뿐입니다. 중요한 건 이 뉴스가 실제로 어떤 자산을 먼저 움직였는지입니다.

## 본문

## 1. 지금 무슨 일이 있었나

이번 글의 출발점은 `U.S. House panel shares crypto tax bill ahead of hearing later this week`입니다. 다만 제목만 읽고 끝내면 가장 중요한 부분을 놓치기 쉽습니다.
...
- 주요 참고 소스: CoinDesk RSS, Cointelegraph, Financial Times Home, Financial Times World, Investing.com Crypto News, MarketWatch Breaking News
...
- keyword `ai_semiconductors` / publish `2026-09-17` / priority `98.0`
- review `approve` score `100` / quality `pass` / ready_now `True`
- intent: 당일 이슈가 내 투자에 어떤 영향을 주는지 빠르게 이해하고 싶은 독자
...
예를 들어 `Kraken brings DeFi yield to tokenized stocks and ETFs` 같은 제목은 출발점일 뿐입니다. 중요한 건 이 뉴스가 실제로 어떤 자산을 먼저 움직였는지입니다.

## 본문

## 1. 지금 무슨 일이 있었나

이번 글의 출발점은 `Kraken brings DeFi yield to tokenized stocks and ETFs`입니다. 다만 제목만 읽고 끝내면 가장 중요한 부분을 놓치기 쉽습니다.
...
- keyword `china` / publish `2026-09-18` / priority `100.0`
- review `approve` score `100` / quality `pass` / ready_now `True`
- intent: 당일 이슈가 내 투자에 어떤 영향을 주는지 빠르게 이해하고 싶은 독자
- CTA focus: 실적·공급망·대표 종목 글로 연결
- reason: 복수 소스 교차 확인 가능 (3개), 섹터/세계 흐름 연결 해설 가능
...
예를 들어 `Trump goes scorched earth on AI warnings, raging about data center opposition and regulation` 같은 제목은 출발점일 뿐입니다. 중요한 건 이 뉴스가 실제로 어떤 자산을 먼저 움직였는지입니다.

## 본문

## 1. 지금 무슨 일이 있었나

이번 글의 출발점은 `Trump goes scorched earth on AI warnings, raging about data center opposition and regulation`입니다. 다만 제목만 읽고 끝내면 가장 중요한 부분을 놓치기 쉽습니다.

...
- batch confirmation: `python3 /home/runner/work/investment-blog-cloud-sync/investment-blog-cloud-sync/scripts/set_review_approvals.py --keywords bitcoin fomc`

## 1. 비트코인 가격보다 먼저 봐야 할 것: ETF 자금, 달러, 규제 체크포인트

- keyword `bitcoin` / publish `2026-09-21` / priority `123.0`
...
- evidence score: demand `5200` / fallback `source_snapshot_rank` / format `crypto_analysis`
...
- recent_evidence:
  ...
  - Investing.com Crypto News | 2026-09-20 07:02:23 | Bitcoin trapped between $78.5K-$82K in breakout standoff: Live
  - Investing.com Crypto News | 2026-09-20 05:06:36 | Bitcoin holds above $80,000 as ETF inflows revive institutional demand
  - Cointelegraph | 2026-09-19T19:18:00+00:00 | REX launches 2x leveraged ETF tied to Bitcoin treasury firm Strive
- image_slots:
...
2026년 9월 20일 기준, 이 이슈를 그냥 뉴스 하나로 넘기기엔 아깝습니다.
...
예를 들어 `Crypto platform Gemini’s stock is down 80% from its IPO. That’s reviving takeover speculation` 같은 제목은 출발점일 뿐입니다. 중요한 건 이 뉴스가 실제로 어떤 자산을 먼저 움직였는지입니다.

## 본문

## 1. 지금 무슨 일이 있었나

이번 글의 출발점은 `Crypto platform Gemini’s stock is down 80% from its IPO. That’s reviving takeover speculation`입니다. 다만 제목만 읽고 끝내면 가장 중요한 부분을 놓치기 쉽습니다.
...
- keyword `ai_semiconductors` / publish `2026-09-22` / priority `97.0`
- review `approve` score `100` / quality `pass` / ready_now `True`
- intent: 당일 이슈가 내 투자에 어떤 영향을 주는지 빠르게 이해하고 싶은 독자
...
- source_names: CNBC Top News, Cointelegraph, Financial Times YouTube, Reuters Markets via Google News RSS
- sample_headlines:
...
- recent_evidence:
  ...
  - CNBC Top News | 2026-09-20T11:25:08+00:00 | Nvidia CEO Jensen Huang emerges as Trump's top ally in AI safety debate
  - Cointelegraph | 2026-09-20T07:51:24+00:00 | Anthropic tabs Accenture as embedded evaluator to help with AI slowdown proposal
  - Reuters Markets via Google News RSS | 2026-09-19T00:05:00+00:00 | EXCLUSIVE: Anthropic considers releasing new AI model ahead of IPO, sources say - Reuters
- image_slots:
...
예를 들어 `Anthropic tabs Accenture as embedded evaluator to help with AI slowdown proposal` 같은 제목은 출발점일 뿐입니다. 중요한 건 이 뉴스가 실제로 어떤 자산을 먼저 움직였는지입니다.

## 본문

## 1. 지금 무슨 일이 있었나

이번 글의 출발점은 `Anthropic tabs Accenture as embedded evaluator to help with AI slowdown proposal`입니다. 다만 제목만 읽고 끝내면 가장 중요한 부분을 놓치기 쉽습니다.
...
- 주요 참고 소스: CNBC Top News, Cointelegraph, Financial Times YouTube, Reuters Markets via Google News RSS
...
- keyword `fomc` / publish `2026-09-20` / priority `133.0`
- review `approve` score `100` / quality `pass` / ready_now `True`
- intent: 당일 이슈가 내 투자에 어떤 영향을 주는지 빠르게 이해하고 싶은 독자
- CTA focus: 환율·금리·미국증시 evergreen 글로 연결
- reason: 공식 소스 기반 확인 가능, 복수 소스 교차 확인 가능 (2개), 거시 해설형 글로 전환 가치 높음
- evidence score: demand `3400` / fallback `source_snapshot_rank` / format `macro_explainer`
- source_names: Federal Reserve Monetary Policy Press, MarketWatch Breaking News
- sample_headlines:

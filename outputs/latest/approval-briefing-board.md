...
- item_count: `3`
- single confirmation: `python3 /home/runner/work/investment-blog-cloud-sync/investment-blog-cloud-sync/scripts/set_review_approvals.py --keywords treasury_yields`
- batch confirmation: `python3 /home/runner/work/investment-blog-cloud-sync/investment-blog-cloud-sync/scripts/set_review_approvals.py --keywords treasury_yields bitcoin`

## 1. 미국채 금리 상승이 나스닥과 코인에 부담이 되는 이유

- keyword `treasury_yields` / publish `2026-09-02` / priority `130.0`
...
- source_names: CoinDesk RSS, Financial Times Home, Financial Times World, MarketWatch Breaking News, NYT Business, Reuters Markets via Google News RSS
- sample_headlines:
...
- recent_evidence:
  ...
  - MarketWatch Breaking News | 2026-09-01T20:36:00+00:00 | This could be the 10-year Treasury’s tipping point into the danger zone
- image_slots:
...
2026년 9월 2일 기준, 이 이슈를 그냥 뉴스 하나로 넘기기엔 아깝습니다.
...
예를 들어 `Firelight raises $8 million, expands beyond XRP as it aims to make DeFi less scary for fintechs` 같은 제목은 출발점일 뿐입니다. 중요한 건 이 뉴스가 실제로 어떤 자산을 먼저 움직였는지입니다.

## 본문

## 1. 지금 무슨 일이 있었나

이번 글의 출발점은 `Firelight raises $8 million, expands beyond XRP as it aims to make DeFi less scary for fintechs`입니다. 다만 제목만 읽고 끝내면 가장 중요한 부분을 놓치기 쉽습니다.
...
- 주요 참고 소스: CoinDesk RSS, Financial Times Home, Financial Times World, MarketWatch Breaking News, NYT Business, Reuters Markets via Google News RSS
...
- keyword `bitcoin` / publish `2026-09-03` / priority `119.0`
- review `approve` score `100` / quality `pass` / ready_now `True`
- intent: 당일 이슈가 내 투자에 어떤 영향을 주는지 빠르게 이해하고 싶은 독자
- CTA focus: ETF·규제·초보 가이드 글로 연결
- reason: 복수 소스 교차 확인 가능 (3개), 코인 독자 유입과 재방문 가능성, 코인 시장 신호 반영 (mixed)
- evidence score: demand `4700` / fallback `source_snapshot_rank` / format `crypto_analysis`
...
- recent_evidence:
  ...
  - Cointelegraph | 2026-09-01T21:25:27+00:00 | Crypto-backed PAC scales back ad spending in Massachusetts primary
- image_slots:
...
예를 들어 `Robinhood's new crypto network is printing cash, and it's sending Arbitrum's token soaring` 같은 제목은 출발점일 뿐입니다. 중요한 건 이 뉴스가 실제로 어떤 자산을 먼저 움직였는지입니다.

## 본문

## 1. 지금 무슨 일이 있었나

이번 글의 출발점은 `Robinhood's new crypto network is printing cash, and it's sending Arbitrum's token soaring`입니다. 다만 제목만 읽고 끝내면 가장 중요한 부분을 놓치기 쉽습니다.
...
- keyword `us_big_tech` / publish `2026-09-04` / priority `102.0`
- review `approve` score `100` / quality `pass` / ready_now `True`
- intent: 당일 이슈가 내 투자에 어떤 영향을 주는지 빠르게 이해하고 싶은 독자
...
- source_names: CNBC Top News, Financial Times Home, Financial Times World, MarketWatch Breaking News
- sample_headlines:
...
예를 들어 `Dell surges 9% after lifting fiscal 2027 forecast on AI server strength` 같은 제목은 출발점일 뿐입니다. 중요한 건 이 뉴스가 실제로 어떤 자산을 먼저 움직였는지입니다.

## 본문

## 1. 지금 무슨 일이 있었나

이번 글의 출발점은 `Dell surges 9% after lifting fiscal 2027 forecast on AI server strength`입니다. 다만 제목만 읽고 끝내면 가장 중요한 부분을 놓치기 쉽습니다.
...
- 주요 참고 소스: CNBC Top News, Financial Times Home, Financial Times World, MarketWatch Breaking News

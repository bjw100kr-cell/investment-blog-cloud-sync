...
- keyword `bitcoin` / publish `2026-09-08` / priority `122.0`
...
- recent_evidence:
  ...
  - Investing.com Crypto News | 2026-09-06 19:02:11 | Bitcoin consolidates near $79,955 with overbought signals: Live levels
- image_slots:
...
예를 들어 `Bitcoin network used by exchanges hit by $320 million exploit. Hackers claim they're the 'good guys'` 같은 제목은 출발점일 뿐입니다. 중요한 건 이 뉴스가 실제로 어떤 자산을 먼저 움직였는지입니다.

## 본문

## 1. 지금 무슨 일이 있었나

이번 글의 출발점은 `Bitcoin network used by exchanges hit by $320 million exploit. Hackers claim they're the 'good guys'`입니다. 다만 제목만 읽고 끝내면 가장 중요한 부분을 놓치기 쉽습니다.
...
- keyword `ai_semiconductors` / publish `2026-09-09` / priority `103.0`
- review `approve` score `100` / quality `pass` / ready_now `True`
- intent: 당일 이슈가 내 투자에 어떤 영향을 주는지 빠르게 이해하고 싶은 독자
...
- source_names: Financial Times YouTube, Google Trends KR, NYT Business
- sample_headlines:
  - 반도체 메모리
...
예를 들어 `반도체 메모리` 같은 제목은 출발점일 뿐입니다. 중요한 건 이 뉴스가 실제로 어떤 자산을 먼저 움직였는지입니다.

## 본문

## 1. 지금 무슨 일이 있었나

이번 글의 출발점은 `반도체 메모리`입니다. 다만 제목만 읽고 끝내면 가장 중요한 부분을 놓치기 쉽습니다.
...
- 주요 참고 소스: Financial Times YouTube, Google Trends KR, NYT Business
...
- keyword `china` / publish `2026-09-10` / priority `68.0`
...
예를 들어 `How a Blacklisted Chinese Tech Giant Kept Buying America’s Best A.I. Chips` 같은 제목은 출발점일 뿐입니다. 중요한 건 이 뉴스가 실제로 어떤 자산을 먼저 움직였는지입니다.

## 본문

## 1. 지금 무슨 일이 있었나

이번 글의 출발점은 `How a Blacklisted Chinese Tech Giant Kept Buying America’s Best A.I. Chips`입니다. 다만 제목만 읽고 끝내면 가장 중요한 부분을 놓치기 쉽습니다.

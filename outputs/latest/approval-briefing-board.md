...
- keyword `bitcoin` / publish `2026-07-28` / priority `122.0`
...
- recent_evidence:
  ...
  - Investing.com Crypto News | 2026-07-27 07:06:46 | Bitcoin bull flag 85% complete at $65,414: Live levels
- image_slots:
...
예를 들어 `Fanatics buys regulated exchange in bid to grow prediction markets business` 같은 제목은 출발점일 뿐입니다. 중요한 건 이 뉴스가 실제로 어떤 자산을 먼저 움직였는지입니다.

## 본문

## 1. 지금 무슨 일이 있었나

이번 글의 출발점은 `Fanatics buys regulated exchange in bid to grow prediction markets business`입니다. 다만 제목만 읽고 끝내면 가장 중요한 부분을 놓치기 쉽습니다.
...
- keyword `ai_semiconductors` / publish `2026-07-29` / priority `105.0`
- review `approve` score `100` / quality `pass` / ready_now `True`
- intent: 당일 이슈가 내 투자에 어떤 영향을 주는지 빠르게 이해하고 싶은 독자
...
- source_names: CNBC Top News, CoinDesk RSS, Financial Times Home, Financial Times YouTube, Reuters Markets via Google News RSS
- sample_headlines:
...
- recent_evidence:
  ...
  - CNBC Top News | 2026-07-27T11:32:45+00:00 | Nvidia, SpaceX, Microsoft launch AI safety initiative as OpenAI cyberattack fallout continues
- image_slots:
...
예를 들어 `Nvidia forms 37-member AI security alliance without OpenAI, Anthropic or Google` 같은 제목은 출발점일 뿐입니다. 중요한 건 이 뉴스가 실제로 어떤 자산을 먼저 움직였는지입니다.

## 본문

## 1. 지금 무슨 일이 있었나

이번 글의 출발점은 `Nvidia forms 37-member AI security alliance without OpenAI, Anthropic or Google`입니다. 다만 제목만 읽고 끝내면 가장 중요한 부분을 놓치기 쉽습니다.
...
- 주요 참고 소스: CNBC Top News, CoinDesk RSS, Financial Times Home, Financial Times YouTube, Reuters Markets via Google News RSS
...
- keyword `china` / publish `2026-07-30` / priority `117.0`
- review `approve` score `100` / quality `pass` / ready_now `True`
- intent: 당일 이슈가 내 투자에 어떤 영향을 주는지 빠르게 이해하고 싶은 독자
- CTA focus: 실적·공급망·대표 종목 글로 연결
- reason: 복수 소스 교차 확인 가능 (5개), 섹터/세계 흐름 연결 해설 가능
- evidence score: demand `3200` / fallback `source_snapshot_rank` / format `macro_explainer`
- source_names: CNBC Top News, Financial Times Home, NYT Business, Reuters Markets via Google News RSS, 무역킹 Trade King YouTube
- sample_headlines:
...
- recent_evidence:
  ...
  - Reuters Markets via Google News RSS | 2026-07-26T23:44:00+00:00 | Chipmaker CXMT vaults to top of China's valuation with 466% surge in Shanghai debut - Reuters
- image_slots:
...
예를 들어 `Why Trump's new tariff blitz is very different this time around` 같은 제목은 출발점일 뿐입니다. 중요한 건 이 뉴스가 실제로 어떤 자산을 먼저 움직였는지입니다.

## 본문

## 1. 지금 무슨 일이 있었나

이번 글의 출발점은 `Why Trump's new tariff blitz is very different this time around`입니다. 다만 제목만 읽고 끝내면 가장 중요한 부분을 놓치기 쉽습니다.
...
- 주요 참고 소스: CNBC Top News, Financial Times Home, NYT Business, Reuters Markets via Google News RSS, 무역킹 Trade King YouTube

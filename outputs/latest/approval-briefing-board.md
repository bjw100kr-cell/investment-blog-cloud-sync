...
- keyword `fomc` / publish `2026-09-14` / priority `139.0`
...
- evidence score: demand `5300` / fallback `source_snapshot_rank` / format `macro_explainer`
- source_names: CNBC Top News, Federal Reserve Monetary Policy Press, Investing.com Crypto News, Reuters Markets via Google News RSS
- sample_headlines:
...
2026년 9월 14일 기준, 이 이슈를 그냥 뉴스 하나로 넘기기엔 아깝습니다.
...
- 주요 참고 소스: CNBC Top News, Federal Reserve Monetary Policy Press, Investing.com Crypto News, Reuters Markets via Google News RSS
...
- keyword `bitcoin` / publish `2026-09-15` / priority `127.0`
- review `approve` score `100` / quality `pass` / ready_now `True`
- intent: 당일 이슈가 내 투자에 어떤 영향을 주는지 빠르게 이해하고 싶은 독자
- CTA focus: ETF·규제·초보 가이드 글로 연결
- reason: 복수 소스 교차 확인 가능 (6개), 코인 독자 유입과 재방문 가능성, 코인 시장 신호 반영 (mixed)
- evidence score: demand `6900` / fallback `source_snapshot_rank` / format `crypto_analysis`
- source_names: CNBC Top News, CoinDesk RSS, Cointelegraph, Investing.com Crypto News, MarketWatch Breaking News, Reuters Markets via Google News RSS
- sample_headlines:
...
예를 들어 `Banks escalate stablecoin rewards fight as Senate prepares for a Clarity Act vote` 같은 제목은 출발점일 뿐입니다. 중요한 건 이 뉴스가 실제로 어떤 자산을 먼저 움직였는지입니다.

## 본문

## 1. 지금 무슨 일이 있었나

이번 글의 출발점은 `Banks escalate stablecoin rewards fight as Senate prepares for a Clarity Act vote`입니다. 다만 제목만 읽고 끝내면 가장 중요한 부분을 놓치기 쉽습니다.
...
- 주요 참고 소스: CNBC Top News, CoinDesk RSS, Cointelegraph, Investing.com Crypto News, MarketWatch Breaking News, Reuters Markets via Google News RSS
...
- keyword `ai_semiconductors` / publish `2026-09-16` / priority `119.0`
- review `approve` score `100` / quality `pass` / ready_now `True`
- intent: 당일 이슈가 내 투자에 어떤 영향을 주는지 빠르게 이해하고 싶은 독자
...
- source_names: CNBC Top News, Financial Times Home, Financial Times World, Financial Times YouTube, Reuters Markets via Google News RSS
- sample_headlines:
...
- recent_evidence:
  ...
  - CNBC Top News | 2026-09-14T14:52:38+00:00 | What Amodei's AI slowdown could mean for Anthropic's imminent IPO
- image_slots:
...
예를 들어 `Trump says no need for more AI regulation, slams Anthropic CEO Dario Amodei` 같은 제목은 출발점일 뿐입니다. 중요한 건 이 뉴스가 실제로 어떤 자산을 먼저 움직였는지입니다.

## 본문

## 1. 지금 무슨 일이 있었나

이번 글의 출발점은 `Trump says no need for more AI regulation, slams Anthropic CEO Dario Amodei`입니다. 다만 제목만 읽고 끝내면 가장 중요한 부분을 놓치기 쉽습니다.
...
- 주요 참고 소스: CNBC Top News, Financial Times Home, Financial Times World, Financial Times YouTube, Reuters Markets via Google News RSS

...
- keyword `bitcoin` / publish `2026-08-19` / priority `122.0`
...
- recent_evidence:
  ...
  - Cointelegraph | 2026-08-18T09:31:15+00:00 | Bitcoin price spike to $64.5K was ‘low-volume liquidity trap’: Analysis
- image_slots:
...
예를 들어 `The 'crack' in the energy market is wider than ever. Bitcoin might feel it.` 같은 제목은 출발점일 뿐입니다. 중요한 건 이 뉴스가 실제로 어떤 자산을 먼저 움직였는지입니다.

## 본문

## 1. 지금 무슨 일이 있었나

이번 글의 출발점은 `The 'crack' in the energy market is wider than ever. Bitcoin might feel it.`입니다. 다만 제목만 읽고 끝내면 가장 중요한 부분을 놓치기 쉽습니다.
...
- keyword `ai_semiconductors` / publish `2026-08-20` / priority `82.0`
- review `approve` score `100` / quality `pass` / ready_now `True`
- intent: 당일 이슈가 내 투자에 어떤 영향을 주는지 빠르게 이해하고 싶은 독자
...
- source_names: CNBC Top News, Financial Times YouTube
- sample_headlines:
...
예를 들어 `Nvidia's AI moat is shifting from chips to capital` 같은 제목은 출발점일 뿐입니다. 중요한 건 이 뉴스가 실제로 어떤 자산을 먼저 움직였는지입니다.

## 본문

## 1. 지금 무슨 일이 있었나

이번 글의 출발점은 `Nvidia's AI moat is shifting from chips to capital`입니다. 다만 제목만 읽고 끝내면 가장 중요한 부분을 놓치기 쉽습니다.
...
- 주요 참고 소스: CNBC Top News, Financial Times YouTube
...
- keyword `china` / publish `2026-08-21` / priority `79.0`
- review `approve` score `100` / quality `pass` / ready_now `True`
- intent: 당일 이슈가 내 투자에 어떤 영향을 주는지 빠르게 이해하고 싶은 독자
- CTA focus: 실적·공급망·대표 종목 글로 연결
- reason: 복수 소스 교차 확인 가능 (2개), 섹터/세계 흐름 연결 해설 가능
...
예를 들어 `China adds 8 banks to digital yuan network as operator count hits 30` 같은 제목은 출발점일 뿐입니다. 중요한 건 이 뉴스가 실제로 어떤 자산을 먼저 움직였는지입니다.

## 본문

## 1. 지금 무슨 일이 있었나

이번 글의 출발점은 `China adds 8 banks to digital yuan network as operator count hits 30`입니다. 다만 제목만 읽고 끝내면 가장 중요한 부분을 놓치기 쉽습니다.

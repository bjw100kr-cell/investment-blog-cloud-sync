...
- keyword `fomc` / publish `2026-07-21` / priority `128.0`
...
- keyword `bitcoin` / publish `2026-07-22` / priority `131.0`
- review `approve` score `100` / quality `pass` / ready_now `True`
- intent: 당일 이슈가 내 투자에 어떤 영향을 주는지 빠르게 이해하고 싶은 독자
- CTA focus: ETF·규제·초보 가이드 글로 연결
- reason: 검색 트렌드 반응 존재, 복수 소스 교차 확인 가능 (5개), 코인 독자 유입과 재방문 가능성, 코인 시장 신호 반영 (extreme_fear), 실제 급상승 검색어 반영 (비트코인, fidelity bitcoin)
- evidence score: demand `2500` / fallback `trend_match` / format `crypto_analysis`
- source_names: CoinDesk RSS, Cointelegraph, Google Trends KR, Google Trends US, Investing.com Crypto News
- sample_headlines:
  - 비트코인
  - fidelity bitcoin
...
- recent_evidence:
  ...
  - Google Trends US | 2026-07-21T01:40:00-07:00 | fidelity bitcoin
- image_slots:
...
- 주요 참고 소스: CoinDesk RSS, Cointelegraph, Google Trends KR, Google Trends US, Investing.com Crypto News
...
- keyword `ai_semiconductors` / publish `2026-07-23` / priority `91.0`
- review `approve` score `100` / quality `pass` / ready_now `True`
- intent: 당일 이슈가 내 투자에 어떤 영향을 주는지 빠르게 이해하고 싶은 독자
...
- source_names: CoinDesk RSS, Financial Times Home, Financial Times YouTube
- sample_headlines:
...
예를 들어 `Bitcoin hits a two-week high near $65,500 as the chip trade turns back into a tailwind` 같은 제목은 출발점일 뿐입니다. 중요한 건 이 뉴스가 실제로 어떤 자산을 먼저 움직였는지입니다.

## 본문

## 1. 지금 무슨 일이 있었나

이번 글의 출발점은 `Bitcoin hits a two-week high near $65,500 as the chip trade turns back into a tailwind`입니다. 다만 제목만 읽고 끝내면 가장 중요한 부분을 놓치기 쉽습니다.
...
- 주요 참고 소스: CoinDesk RSS, Financial Times Home, Financial Times YouTube

...
- keyword `fomc` / publish `2026-09-24` / priority `137.0`
...
- source_names: CNBC Top News, Federal Reserve Monetary Policy Press, Financial Times Home, Reuters Markets via Google News RSS
- sample_headlines:
...
2026년 9월 24일 기준, 이 이슈를 그냥 뉴스 하나로 넘기기엔 아깝습니다.
...
- 주요 참고 소스: CNBC Top News, Federal Reserve Monetary Policy Press, Financial Times Home, Reuters Markets via Google News RSS
...
- keyword `bitcoin` / publish `2026-09-25` / priority `124.0`
- review `approve` score `100` / quality `pass` / ready_now `True`
- intent: 당일 이슈가 내 투자에 어떤 영향을 주는지 빠르게 이해하고 싶은 독자
...
- recent_evidence:
  ...
  - Investing.com Crypto News | 2026-09-23 19:18:30 | Bitcoin pinned at $84,328 as bearish momentum builds: Live levels
- image_slots:
...
예를 들어 `Dogecoin down 8%, bitcoin under $84,000 as Treasury yields hit highest level since 2007` 같은 제목은 출발점일 뿐입니다. 중요한 건 이 뉴스가 실제로 어떤 자산을 먼저 움직였는지입니다.

## 본문

## 1. 지금 무슨 일이 있었나

이번 글의 출발점은 `Dogecoin down 8%, bitcoin under $84,000 as Treasury yields hit highest level since 2007`입니다. 다만 제목만 읽고 끝내면 가장 중요한 부분을 놓치기 쉽습니다.
...
- keyword `ai_semiconductors` / publish `2026-09-26` / priority `85.0`
...
예를 들어 `Micron’s stock has become a ‘battleground’ as the AI narrative shifts` 같은 제목은 출발점일 뿐입니다. 중요한 건 이 뉴스가 실제로 어떤 자산을 먼저 움직였는지입니다.

## 본문

## 1. 지금 무슨 일이 있었나

이번 글의 출발점은 `Micron’s stock has become a ‘battleground’ as the AI narrative shifts`입니다. 다만 제목만 읽고 끝내면 가장 중요한 부분을 놓치기 쉽습니다.

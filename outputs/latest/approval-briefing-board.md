...
- keyword `fomc` / publish `2026-09-05` / priority `140.0`
...
- evidence score: demand `5600` / fallback `source_snapshot_rank` / format `macro_explainer`
- source_names: CNBC Top News, CoinDesk RSS, Cointelegraph, Federal Reserve Monetary Policy Press, Investing.com Crypto News, NYT Business, Reuters Markets via Google News RSS
- sample_headlines:
...
2026년 9월 5일 기준, 이 이슈를 그냥 뉴스 하나로 넘기기엔 아깝습니다.
...
- 주요 참고 소스: CNBC Top News, CoinDesk RSS, Cointelegraph, Federal Reserve Monetary Policy Press, Investing.com Crypto News, NYT Business, Reuters Markets via Google News RSS
...
- keyword `bitcoin` / publish `2026-09-06` / priority `122.0`
- review `approve` score `100` / quality `pass` / ready_now `True`
- intent: 당일 이슈가 내 투자에 어떤 영향을 주는지 빠르게 이해하고 싶은 독자
...
- recent_evidence:
  ...
  - Investing.com Crypto News | 2026-09-05 09:24:34 | Bitcoin falls below $80,000 as strong U.S. jobs data revives Fed rate hike fears
  - Investing.com Crypto News | 2026-09-05 07:02:04 | Bitcoin stuck in $4K range with Doji signal: Live levels
- image_slots:
...
예를 들어 `British investor thought he lost $2,000 in bitcoin in 2012. He just recovered $4.5 million` 같은 제목은 출발점일 뿐입니다. 중요한 건 이 뉴스가 실제로 어떤 자산을 먼저 움직였는지입니다.

## 본문

## 1. 지금 무슨 일이 있었나

이번 글의 출발점은 `British investor thought he lost $2,000 in bitcoin in 2012. He just recovered $4.5 million`입니다. 다만 제목만 읽고 끝내면 가장 중요한 부분을 놓치기 쉽습니다.
...
- keyword `ai_semiconductors` / publish `2026-09-07` / priority `81.0`
- review `approve` score `100` / quality `pass` / ready_now `True`
- intent: 당일 이슈가 내 투자에 어떤 영향을 주는지 빠르게 이해하고 싶은 독자

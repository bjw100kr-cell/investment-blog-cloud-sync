...
- keyword `bitcoin` / publish `2026-07-02` / priority `128.0`
- review `approve` score `100` / quality `pass` / ready_now `True`
- intent: 당일 이슈가 내 투자에 어떤 영향을 주는지 빠르게 이해하고 싶은 독자
- CTA focus: ETF·규제·초보 가이드 글로 연결
- reason: 복수 소스 교차 확인 가능 (5개), 코인 독자 유입과 재방문 가능성, 코인 시장 신호 반영 (extreme_fear)
...
- recent_evidence:
  ...
  - CoinDesk RSS | 2026-07-01T07:42:49+00:00 | Bitcoin’s 20% June crash looks even deadlier on the charts. Here’s why
- image_slots:
...
예를 들어 `XRP, HYPE funds are the bright spots as investors flee bitcoin, ether ETFs` 같은 제목은 출발점일 뿐입니다. 중요한 건 이 뉴스가 실제로 어떤 자산을 먼저 움직였는지입니다.

## 본문

## 1. 지금 무슨 일이 있었나

이번 글의 출발점은 `XRP, HYPE funds are the bright spots as investors flee bitcoin, ether ETFs`입니다. 다만 제목만 읽고 끝내면 가장 중요한 부분을 놓치기 쉽습니다.
...
- keyword `ai_semiconductors` / publish `2026-07-03` / priority `118.0`
- review `approve` score `100` / quality `pass` / ready_now `True`
- intent: 당일 이슈가 내 투자에 어떤 영향을 주는지 빠르게 이해하고 싶은 독자
...
- source_names: CNBC Top News, CoinDesk RSS, Cointelegraph, Financial Times Home, Financial Times World, MarketWatch Breaking News, NYT Business
- sample_headlines:
...
- recent_evidence:
  ...
  - CoinDesk RSS | 2026-07-01T06:54:13+00:00 | Anthropic restores AI models Fable, Mythos after the U.S. lifts export controls
- image_slots:
...
쉽게 말해 해외 주요 매체 보도, 코인 전문 매체 기사를 같이 보면 headline 뒤에 있는 자금 흐름과 심리 변화를 더 빨리 잡을 수 있습니다.
예를 들어 `Anthropic restores AI models Fable, Mythos after the U.S. lifts export controls` 같은 제목은 출발점일 뿐입니다. 중요한 건 이 뉴스가 실제로 어떤 자산을 먼저 움직였는지입니다.

## 본문

## 1. 지금 무슨 일이 있었나

이번 글의 출발점은 `Anthropic restores AI models Fable, Mythos after the U.S. lifts export controls`입니다. 다만 제목만 읽고 끝내면 가장 중요한 부분을 놓치기 쉽습니다.
...
- keyword `china` / publish `2026-07-04` / priority `102.0`
- review `approve` score `100` / quality `pass` / ready_now `True`
- intent: 당일 이슈가 내 투자에 어떤 영향을 주는지 빠르게 이해하고 싶은 독자
- CTA focus: 실적·공급망·대표 종목 글로 연결
- reason: 복수 소스 교차 확인 가능 (4개), 섹터/세계 흐름 연결 해설 가능
- evidence score: demand `0` / fallback `mapped_candidate` / format `macro_explainer`
- source_names: CNBC Top News, Financial Times World, NYT Business, 무역킹 Trade King YouTube
- sample_headlines:
...
예를 들어 `Bulls are betting big on this global ETF that's deep in a bear market` 같은 제목은 출발점일 뿐입니다. 중요한 건 이 뉴스가 실제로 어떤 자산을 먼저 움직였는지입니다.

## 본문

## 1. 지금 무슨 일이 있었나

이번 글의 출발점은 `Bulls are betting big on this global ETF that's deep in a bear market`입니다. 다만 제목만 읽고 끝내면 가장 중요한 부분을 놓치기 쉽습니다.
...
- 주요 참고 소스: CNBC Top News, Financial Times World, NYT Business, 무역킹 Trade King YouTube

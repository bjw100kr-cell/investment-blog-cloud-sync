...
- keyword `fomc` / publish `2026-08-26` / priority `139.0`
...
- source_names: CNBC Top News, CoinDesk RSS, Federal Reserve Monetary Policy Press, Financial Times Home, Financial Times World, MarketWatch Breaking News, NYT Business
- sample_headlines:
...
- 주요 참고 소스: CNBC Top News, CoinDesk RSS, Federal Reserve Monetary Policy Press, Financial Times Home, Financial Times World, MarketWatch Breaking News, NYT Business
...
- keyword `bitcoin` / publish `2026-08-27` / priority `121.0`
- review `approve` score `100` / quality `pass` / ready_now `True`
- intent: 당일 이슈가 내 투자에 어떤 영향을 주는지 빠르게 이해하고 싶은 독자
...
- recent_evidence:
  ...
  - CoinDesk RSS | 2026-08-26T10:45:29+00:00 | Bitcoin takes a breather after adding 23% in 7 days as ETF demand holds steady
- image_slots:
...
예를 들어 `The 3 catalysts that could define bitcoin's next move` 같은 제목은 출발점일 뿐입니다. 중요한 건 이 뉴스가 실제로 어떤 자산을 먼저 움직였는지입니다.

## 본문

## 1. 지금 무슨 일이 있었나

이번 글의 출발점은 `The 3 catalysts that could define bitcoin's next move`입니다. 다만 제목만 읽고 끝내면 가장 중요한 부분을 놓치기 쉽습니다.
...
- keyword `ai_semiconductors` / publish `2026-08-28` / priority `96.0`
- review `approve` score `100` / quality `pass` / ready_now `True`
- intent: 당일 이슈가 내 투자에 어떤 영향을 주는지 빠르게 이해하고 싶은 독자
...
- recent_evidence:
  ...
  - Reuters Markets via Google News RSS | 2026-08-26T02:17:00+00:00 | World stocks inch up as oil drops on Hormuz hopes ahead of data, Nvidia results - Reuters
- image_slots:
...
예를 들어 `World stocks inch up as oil drops on Hormuz hopes ahead of data, Nvidia results - Reuters` 같은 제목은 출발점일 뿐입니다. 중요한 건 이 뉴스가 실제로 어떤 자산을 먼저 움직였는지입니다.

## 본문

## 1. 지금 무슨 일이 있었나

이번 글의 출발점은 `World stocks inch up as oil drops on Hormuz hopes ahead of data, Nvidia results - Reuters`입니다. 다만 제목만 읽고 끝내면 가장 중요한 부분을 놓치기 쉽습니다.

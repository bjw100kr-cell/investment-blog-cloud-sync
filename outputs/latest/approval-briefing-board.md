...
- keyword `fomc` / publish `2026-09-25` / priority `137.0`
...
- source_names: CNBC Top News, CoinDesk RSS, Cointelegraph, Federal Reserve Monetary Policy Press, Investing.com Crypto News
- sample_headlines:
...
- 주요 참고 소스: CNBC Top News, CoinDesk RSS, Cointelegraph, Federal Reserve Monetary Policy Press, Investing.com Crypto News
...
- keyword `bitcoin` / publish `2026-09-26` / priority `119.0`
- review `approve` score `100` / quality `pass` / ready_now `True`
- intent: 당일 이슈가 내 투자에 어떤 영향을 주는지 빠르게 이해하고 싶은 독자
- CTA focus: ETF·규제·초보 가이드 글로 연결
- reason: 복수 소스 교차 확인 가능 (3개), 코인 독자 유입과 재방문 가능성, 코인 시장 신호 반영 (mixed)
- evidence score: demand `4400` / fallback `source_snapshot_rank` / format `crypto_analysis`
...
예를 들어 `Crypto exchange Bitget says $352 million affected in a hack, claims user funds are 'safe'` 같은 제목은 출발점일 뿐입니다. 중요한 건 이 뉴스가 실제로 어떤 자산을 먼저 움직였는지입니다.

## 본문

## 1. 지금 무슨 일이 있었나

이번 글의 출발점은 `Crypto exchange Bitget says $352 million affected in a hack, claims user funds are 'safe'`입니다. 다만 제목만 읽고 끝내면 가장 중요한 부분을 놓치기 쉽습니다.
...
- keyword `us_index_flow` / publish `2026-09-27` / priority `86.0`
...
예를 들어 `S&P 500 ends marginally lower as investors focus on US-Iran war - Reuters` 같은 제목은 출발점일 뿐입니다. 중요한 건 이 뉴스가 실제로 어떤 자산을 먼저 움직였는지입니다.

## 본문

## 1. 지금 무슨 일이 있었나

이번 글의 출발점은 `S&P 500 ends marginally lower as investors focus on US-Iran war - Reuters`입니다. 다만 제목만 읽고 끝내면 가장 중요한 부분을 놓치기 쉽습니다.

...
- keyword `fomc` / publish `2026-09-12` / priority `139.0`
...
- evidence score: demand `5000` / fallback `source_snapshot_rank` / format `macro_explainer`
- source_names: CoinDesk RSS, Federal Reserve Monetary Policy Press, Investing.com Crypto News, MarketWatch Breaking News, Reuters Markets via Google News RSS
- sample_headlines:
...
2026년 9월 12일 기준, 이 이슈를 그냥 뉴스 하나로 넘기기엔 아깝습니다.
...
- 주요 참고 소스: CoinDesk RSS, Federal Reserve Monetary Policy Press, Investing.com Crypto News, MarketWatch Breaking News, Reuters Markets via Google News RSS
...
- keyword `bitcoin` / publish `2026-09-13` / priority `124.0`
- review `approve` score `100` / quality `pass` / ready_now `True`
- intent: 당일 이슈가 내 투자에 어떤 영향을 주는지 빠르게 이해하고 싶은 독자
- CTA focus: ETF·규제·초보 가이드 글로 연결
- reason: 복수 소스 교차 확인 가능 (4개), 코인 독자 유입과 재방문 가능성, 코인 시장 신호 반영 (mixed)
- evidence score: demand `5600` / fallback `source_snapshot_rank` / format `crypto_analysis`
- source_names: CoinDesk RSS, Cointelegraph, Financial Times World, Investing.com Crypto News
- sample_headlines:
...
- recent_evidence:
  ...
  - Investing.com Crypto News | 2026-09-12 09:52:28 | Bitcoin struggles above $77,000 as rising exchange reserves weigh on sentiment
  - Investing.com Crypto News | 2026-09-12 07:02:30 | Bitcoin consolidates above $76,500 support: Live levels
  - Cointelegraph | 2026-09-11T19:46:24+00:00 | Bitcoin Suisse to shift up to half of Swiss jobs abroad
- image_slots:
...
예를 들어 `Bitcoin activity, passports exposed after Revolut falls for fake government request` 같은 제목은 출발점일 뿐입니다. 중요한 건 이 뉴스가 실제로 어떤 자산을 먼저 움직였는지입니다.

## 본문

## 1. 지금 무슨 일이 있었나

이번 글의 출발점은 `Bitcoin activity, passports exposed after Revolut falls for fake government request`입니다. 다만 제목만 읽고 끝내면 가장 중요한 부분을 놓치기 쉽습니다.
...
- 주요 참고 소스: CoinDesk RSS, Cointelegraph, Financial Times World, Investing.com Crypto News
...
- keyword `ai_semiconductors` / publish `2026-09-14` / priority `97.0`
- review `approve` score `100` / quality `pass` / ready_now `True`
- intent: 당일 이슈가 내 투자에 어떤 영향을 주는지 빠르게 이해하고 싶은 독자
...
- source_names: Cointelegraph, Financial Times Home, Financial Times YouTube, MarketWatch Breaking News
- sample_headlines:
...
- recent_evidence:
  ...
  - Cointelegraph | 2026-09-12T15:12:40+00:00 | Nvidia considers $10B investment in potential record Anthropic IPO: Reuters
- image_slots:
...
예를 들어 `Nvidia considers $10B investment in potential record Anthropic IPO: Reuters` 같은 제목은 출발점일 뿐입니다. 중요한 건 이 뉴스가 실제로 어떤 자산을 먼저 움직였는지입니다.

## 본문

## 1. 지금 무슨 일이 있었나

이번 글의 출발점은 `Nvidia considers $10B investment in potential record Anthropic IPO: Reuters`입니다. 다만 제목만 읽고 끝내면 가장 중요한 부분을 놓치기 쉽습니다.
...
- 주요 참고 소스: Cointelegraph, Financial Times Home, Financial Times YouTube, MarketWatch Breaking News

...
- keyword `bitcoin` / publish `2026-07-03` / priority `128.0`
...
- recent_evidence:
  ...
  - CoinDesk RSS | 2026-07-02T06:17:16+00:00 | Bitcoin's long-term holders have returned to accumulation
- image_slots:
...
예를 들어 `Live markets: Bitcoin holds above $60,000 as yen jumps on intervention fears` 같은 제목은 출발점일 뿐입니다. 중요한 건 이 뉴스가 실제로 어떤 자산을 먼저 움직였는지입니다.

## 본문

## 1. 지금 무슨 일이 있었나

이번 글의 출발점은 `Live markets: Bitcoin holds above $60,000 as yen jumps on intervention fears`입니다. 다만 제목만 읽고 끝내면 가장 중요한 부분을 놓치기 쉽습니다.
...
- keyword `ai_semiconductors` / publish `2026-07-04` / priority `111.0`
- review `approve` score `100` / quality `pass` / ready_now `True`
- intent: 당일 이슈가 내 투자에 어떤 영향을 주는지 빠르게 이해하고 싶은 독자
...
- source_names: CNBC Top News, Financial Times Home, MarketWatch Breaking News, NYT Business
- sample_headlines:
...
- recent_evidence:
  ...
  - MarketWatch Breaking News | 2026-07-01T23:11:00+00:00 | Nvidia is betting on a trillion-dollar robotics boom. Here is the hidden way to trade it.
- image_slots:
...
예를 들어 `Nvidia offers start-up customers chance to swap compute power for revenue share` 같은 제목은 출발점일 뿐입니다. 중요한 건 이 뉴스가 실제로 어떤 자산을 먼저 움직였는지입니다.

## 본문

## 1. 지금 무슨 일이 있었나

이번 글의 출발점은 `Nvidia offers start-up customers chance to swap compute power for revenue share`입니다. 다만 제목만 읽고 끝내면 가장 중요한 부분을 놓치기 쉽습니다.
...
- 주요 참고 소스: CNBC Top News, Financial Times Home, MarketWatch Breaking News, NYT Business
...
- keyword `china` / publish `2026-07-05` / priority `66.0`
...
- source_names: 무역킹 Trade King YouTube
- sample_headlines:
...
쉽게 말해 유튜브 해설를 같이 보면 headline 뒤에 있는 자금 흐름과 심리 변화를 더 빨리 잡을 수 있습니다.
예를 들어 `1. The Grand Design to Pressure China` 같은 제목은 출발점일 뿐입니다. 중요한 건 이 뉴스가 실제로 어떤 자산을 먼저 움직였는지입니다.

## 본문

## 1. 지금 무슨 일이 있었나

이번 글의 출발점은 `1. The Grand Design to Pressure China`입니다. 다만 제목만 읽고 끝내면 가장 중요한 부분을 놓치기 쉽습니다.
...
- 주요 참고 소스: 무역킹 Trade King YouTube

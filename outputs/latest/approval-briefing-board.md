...
- item_count: `3`
- single confirmation: `python3 /home/runner/work/investment-blog-cloud-sync/investment-blog-cloud-sync/scripts/set_review_approvals.py --keywords oil`
...
- keyword `oil` / publish `2026-07-04` / priority `105.0`
...
- evidence score: demand `2000` / fallback `source_snapshot_rank` / format `macro_explainer`
...
2026년 7월 4일 기준, 이 이슈를 그냥 뉴스 하나로 넘기기엔 아깝습니다.
...
예를 들어 `Oman walks a diplomatic tightrope over Strait of Hormuz fees, creating a ‘blind spot’ for markets` 같은 제목은 출발점일 뿐입니다. 중요한 건 이 뉴스가 실제로 어떤 자산을 먼저 움직였는지입니다.

## 본문

## 1. 지금 무슨 일이 있었나

이번 글의 출발점은 `Oman walks a diplomatic tightrope over Strait of Hormuz fees, creating a ‘blind spot’ for markets`입니다. 다만 제목만 읽고 끝내면 가장 중요한 부분을 놓치기 쉽습니다.
...
- keyword `bitcoin` / publish `2026-07-05` / priority `127.0`
- review `approve` score `100` / quality `pass` / ready_now `True`
- intent: 당일 이슈가 내 투자에 어떤 영향을 주는지 빠르게 이해하고 싶은 독자
- CTA focus: ETF·규제·초보 가이드 글로 연결
- reason: 복수 소스 교차 확인 가능 (4개), 코인 독자 유입과 재방문 가능성, 코인 시장 신호 반영 (extreme_fear)
- evidence score: demand `7200` / fallback `source_snapshot_rank` / format `crypto_analysis`
...
- recent_evidence:
  ...
  - Cointelegraph | 2026-07-04T03:54:18+00:00 | Bitcoin, Ether extend relief rallies as extreme fear meets renewed ETF buying
- image_slots:
...
예를 들어 `Bitcoin’s next parabolic run may need $1 trillion in fresh capital` 같은 제목은 출발점일 뿐입니다. 중요한 건 이 뉴스가 실제로 어떤 자산을 먼저 움직였는지입니다.

## 본문

## 1. 지금 무슨 일이 있었나

이번 글의 출발점은 `Bitcoin’s next parabolic run may need $1 trillion in fresh capital`입니다. 다만 제목만 읽고 끝내면 가장 중요한 부분을 놓치기 쉽습니다.
...
- keyword `ai_semiconductors` / publish `2026-07-06` / priority `111.0`
- review `approve` score `100` / quality `pass` / ready_now `True`
- intent: 당일 이슈가 내 투자에 어떤 영향을 주는지 빠르게 이해하고 싶은 독자
...
- source_names: CoinDesk RSS, MarketWatch Breaking News, NYT Business, Reuters Markets via Google News RSS
- sample_headlines:
...
예를 들어 `Memory and semiconductor stocks lose momentum, bitcoin rebounds in sign of changing investor focus` 같은 제목은 출발점일 뿐입니다. 중요한 건 이 뉴스가 실제로 어떤 자산을 먼저 움직였는지입니다.

## 본문

## 1. 지금 무슨 일이 있었나

이번 글의 출발점은 `Memory and semiconductor stocks lose momentum, bitcoin rebounds in sign of changing investor focus`입니다. 다만 제목만 읽고 끝내면 가장 중요한 부분을 놓치기 쉽습니다.
...
- 주요 참고 소스: CoinDesk RSS, MarketWatch Breaking News, NYT Business, Reuters Markets via Google News RSS

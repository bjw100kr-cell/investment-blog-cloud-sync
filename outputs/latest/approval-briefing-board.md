...
- keyword `bitcoin` / publish `2026-07-29` / priority `126.0`
...
- recent_evidence:
  ...
  - CoinDesk RSS | 2026-07-28T11:09:16+00:00 | Bitcoin drops as South Korean stocks tumble, Senate shelves crypto Clarity Act
- image_slots:
...
예를 들어 `Core Scientific lands AMD AI deal as bitcoin mining operation winds down` 같은 제목은 출발점일 뿐입니다. 중요한 건 이 뉴스가 실제로 어떤 자산을 먼저 움직였는지입니다.

## 본문

## 1. 지금 무슨 일이 있었나

이번 글의 출발점은 `Core Scientific lands AMD AI deal as bitcoin mining operation winds down`입니다. 다만 제목만 읽고 끝내면 가장 중요한 부분을 놓치기 쉽습니다.
...
- keyword `china` / publish `2026-07-31` / priority `83.0`
- review `approve` score `100` / quality `pass` / ready_now `True`
- intent: 당일 이슈가 내 투자에 어떤 영향을 주는지 빠르게 이해하고 싶은 독자
...
예를 들어 `Micron’s stock sinks toward worst monthly drop in 11 years as China fears escalate` 같은 제목은 출발점일 뿐입니다. 중요한 건 이 뉴스가 실제로 어떤 자산을 먼저 움직였는지입니다.

## 본문

## 1. 지금 무슨 일이 있었나

이번 글의 출발점은 `Micron’s stock sinks toward worst monthly drop in 11 years as China fears escalate`입니다. 다만 제목만 읽고 끝내면 가장 중요한 부분을 놓치기 쉽습니다.
...
- 주요 참고 소스: MarketWatch Breaking News, NYT Business
...
- keyword `us_index_flow` / publish `2026-07-30` / priority `127.0`
...
예를 들어 `stock market` 같은 제목은 출발점일 뿐입니다. 중요한 건 이 뉴스가 실제로 어떤 자산을 먼저 움직였는지입니다.

## 본문

## 1. 지금 무슨 일이 있었나

이번 글의 출발점은 `stock market`입니다. 다만 제목만 읽고 끝내면 가장 중요한 부분을 놓치기 쉽습니다.
...
- 주요 참고 소스: Financial Times Home, Financial Times World, Google Trends US, Reuters Markets via Google News RSS

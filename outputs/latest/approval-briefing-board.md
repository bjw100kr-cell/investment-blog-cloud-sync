...
- keyword `bitcoin` / publish `2026-08-08` / priority `124.0`
...
- recent_evidence:
  ...
  - Cointelegraph | 2026-08-07T11:05:01+00:00 | Binance Bitcoin volume ratio hits record as futures outweigh spot eight times over
- image_slots:
...
예를 들어 `After a Clarity Act funeral, the crypto world would keep turning` 같은 제목은 출발점일 뿐입니다. 중요한 건 이 뉴스가 실제로 어떤 자산을 먼저 움직였는지입니다.

## 본문

## 1. 지금 무슨 일이 있었나

이번 글의 출발점은 `After a Clarity Act funeral, the crypto world would keep turning`입니다. 다만 제목만 읽고 끝내면 가장 중요한 부분을 놓치기 쉽습니다.
...
- keyword `china` / publish `2026-08-10` / priority `90.0`
- review `approve` score `100` / quality `pass` / ready_now `True`
- intent: 당일 이슈가 내 투자에 어떤 영향을 주는지 빠르게 이해하고 싶은 독자
...
예를 들어 `Solar stocks shine after Trump extends China tariffs to polysilicon products` 같은 제목은 출발점일 뿐입니다. 중요한 건 이 뉴스가 실제로 어떤 자산을 먼저 움직였는지입니다.

## 본문

## 1. 지금 무슨 일이 있었나

이번 글의 출발점은 `Solar stocks shine after Trump extends China tariffs to polysilicon products`입니다. 다만 제목만 읽고 끝내면 가장 중요한 부분을 놓치기 쉽습니다.
...
- keyword `ai_semiconductors` / publish `2026-08-09` / priority `107.0`
...
- recent_evidence:
  ...
  - MarketWatch Breaking News | 2026-08-07T02:35:00+00:00 | Micron’s stock falls but is spared the worst of the memory-chip selloff
- image_slots:
...
예를 들어 `amd` 같은 제목은 출발점일 뿐입니다. 중요한 건 이 뉴스가 실제로 어떤 자산을 먼저 움직였는지입니다.

## 본문

## 1. 지금 무슨 일이 있었나

이번 글의 출발점은 `amd`입니다. 다만 제목만 읽고 끝내면 가장 중요한 부분을 놓치기 쉽습니다.
...
- 주요 참고 소스: CNBC Top News, Financial Times YouTube, Google Trends KR, MarketWatch Breaking News

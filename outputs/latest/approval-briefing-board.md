...
- keyword `bitcoin` / publish `2026-08-29` / priority `124.0`
...
- recent_evidence:
  ...
  - CoinDesk RSS | 2026-08-28T05:02:22+00:00 | Bitcoin is trading at a premium on Coinbase after a long time. Here's what it means
- image_slots:
...
2026년 8월 28일 기준, 이 이슈를 그냥 뉴스 하나로 넘기기엔 아깝습니다.
...
예를 들어 `Bitcoin hits highest level in 3 months before pulling back as altcoins consolidate` 같은 제목은 출발점일 뿐입니다. 중요한 건 이 뉴스가 실제로 어떤 자산을 먼저 움직였는지입니다.

## 본문

## 1. 지금 무슨 일이 있었나

이번 글의 출발점은 `Bitcoin hits highest level in 3 months before pulling back as altcoins consolidate`입니다. 다만 제목만 읽고 끝내면 가장 중요한 부분을 놓치기 쉽습니다.
...
- keyword `ai_semiconductors` / publish `2026-08-30` / priority `107.0`
- review `approve` score `100` / quality `pass` / ready_now `True`
- intent: 당일 이슈가 내 투자에 어떤 영향을 주는지 빠르게 이해하고 싶은 독자
- CTA focus: 실적·공급망·대표 종목 글로 연결
- reason: 검색 트렌드 반응 존재, 복수 소스 교차 확인 가능 (4개), 섹터/세계 흐름 연결 해설 가능, 실제 급상승 검색어 반영 (anthropic)
- evidence score: demand `400` / fallback `trend_match` / format `sector_analysis`
- source_names: Financial Times YouTube, Google Trends US, MarketWatch Breaking News, Reuters Markets via Google News RSS
- sample_headlines:
  - anthropic
...
- recent_evidence:
  ...
  - Reuters Markets via Google News RSS | 2026-08-27T23:05:57+00:00 | Nasdaq, S&P 500 lifted by Nvidia's forecast; investors eye speech by Fed's Warsh - Reuters
- image_slots:
...
예를 들어 `anthropic` 같은 제목은 출발점일 뿐입니다. 중요한 건 이 뉴스가 실제로 어떤 자산을 먼저 움직였는지입니다.

## 본문

## 1. 지금 무슨 일이 있었나

이번 글의 출발점은 `anthropic`입니다. 다만 제목만 읽고 끝내면 가장 중요한 부분을 놓치기 쉽습니다.
...
- 주요 참고 소스: Financial Times YouTube, Google Trends US, MarketWatch Breaking News, Reuters Markets via Google News RSS
...
- keyword `china` / publish `2026-08-31` / priority `111.0`
- review `approve` score `100` / quality `pass` / ready_now `True`
- intent: 당일 이슈가 내 투자에 어떤 영향을 주는지 빠르게 이해하고 싶은 독자
- CTA focus: 실적·공급망·대표 종목 글로 연결
...
- recent_evidence:
  ...
  - CNBC Top News | 2026-08-28T01:35:13+00:00 | Trump ratchets up rhetoric against Beijing as U.S.-China officials meet for Xi's Washington visit
- image_slots:
...
예를 들어 `Trump ratchets up rhetoric against Beijing as U.S.-China officials meet for Xi's Washington visit` 같은 제목은 출발점일 뿐입니다. 중요한 건 이 뉴스가 실제로 어떤 자산을 먼저 움직였는지입니다.

## 본문

## 1. 지금 무슨 일이 있었나

이번 글의 출발점은 `Trump ratchets up rhetoric against Beijing as U.S.-China officials meet for Xi's Washington visit`입니다. 다만 제목만 읽고 끝내면 가장 중요한 부분을 놓치기 쉽습니다.

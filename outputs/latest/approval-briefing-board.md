...
- keyword `bitcoin` / publish `2026-08-11` / priority `123.0`
...
- evidence score: demand `8200` / fallback `source_snapshot_rank` / format `crypto_analysis`
...
- recent_evidence:
  ...
  - Cointelegraph | 2026-08-10T11:13:40+00:00 | Markets flip for Fed rate-hike pause into CPI: Five things to know in Bitcoin this week
- image_slots:
...
예를 들어 `Strategy sells 1,690 bitcoin, raises $653 million from MSTR shares` 같은 제목은 출발점일 뿐입니다. 중요한 건 이 뉴스가 실제로 어떤 자산을 먼저 움직였는지입니다.

## 본문

## 1. 지금 무슨 일이 있었나

이번 글의 출발점은 `Strategy sells 1,690 bitcoin, raises $653 million from MSTR shares`입니다. 다만 제목만 읽고 끝내면 가장 중요한 부분을 놓치기 쉽습니다.
...
- keyword `china` / publish `2026-08-13` / priority `74.0`
- review `approve` score `100` / quality `pass` / ready_now `True`
- intent: 당일 이슈가 내 투자에 어떤 영향을 주는지 빠르게 이해하고 싶은 독자
...
예를 들어 `China bypasses shipping chokepoints with ‘Ice Silk Road’ through Arctic` 같은 제목은 출발점일 뿐입니다. 중요한 건 이 뉴스가 실제로 어떤 자산을 먼저 움직였는지입니다.

## 본문

## 1. 지금 무슨 일이 있었나

이번 글의 출발점은 `China bypasses shipping chokepoints with ‘Ice Silk Road’ through Arctic`입니다. 다만 제목만 읽고 끝내면 가장 중요한 부분을 놓치기 쉽습니다.
...
- keyword `ai_semiconductors` / publish `2026-08-12` / priority `114.0`
...
예를 들어 `Meta to open source its most powerful AI model as it takes swipe at OpenAI, Anthropic` 같은 제목은 출발점일 뿐입니다. 중요한 건 이 뉴스가 실제로 어떤 자산을 먼저 움직였는지입니다.

## 본문

## 1. 지금 무슨 일이 있었나

이번 글의 출발점은 `Meta to open source its most powerful AI model as it takes swipe at OpenAI, Anthropic`입니다. 다만 제목만 읽고 끝내면 가장 중요한 부분을 놓치기 쉽습니다.

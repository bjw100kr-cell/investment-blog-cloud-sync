...
- keyword `bitcoin` / publish `2026-08-12` / priority `123.0`
...
- recent_evidence:
  ...
  - Cointelegraph | 2026-08-11T10:55:00+00:00 | Bitcoin sell pressure ‘closer to exhaustion’ after $4B USDT market-cap drop: CryptoQuant
- image_slots:
...
예를 들어 `Pokémon cards are becoming multibillion dollar market. Crypto wants to fix how they trade` 같은 제목은 출발점일 뿐입니다. 중요한 건 이 뉴스가 실제로 어떤 자산을 먼저 움직였는지입니다.

## 본문

## 1. 지금 무슨 일이 있었나

이번 글의 출발점은 `Pokémon cards are becoming multibillion dollar market. Crypto wants to fix how they trade`입니다. 다만 제목만 읽고 끝내면 가장 중요한 부분을 놓치기 쉽습니다.
...
- keyword `ai_semiconductors` / publish `2026-08-13` / priority `109.0`
- review `approve` score `100` / quality `pass` / ready_now `True`
- intent: 당일 이슈가 내 투자에 어떤 영향을 주는지 빠르게 이해하고 싶은 독자
...
- source_names: CNBC Top News, Cointelegraph, Financial Times YouTube, NYT Business, 무역킹 Trade King YouTube
- sample_headlines:
...
예를 들어 `Anthropic strikes $9B compute deal with Bitcoin miner Riot: Report` 같은 제목은 출발점일 뿐입니다. 중요한 건 이 뉴스가 실제로 어떤 자산을 먼저 움직였는지입니다.

## 본문

## 1. 지금 무슨 일이 있었나

이번 글의 출발점은 `Anthropic strikes $9B compute deal with Bitcoin miner Riot: Report`입니다. 다만 제목만 읽고 끝내면 가장 중요한 부분을 놓치기 쉽습니다.
...
- 주요 참고 소스: CNBC Top News, Cointelegraph, Financial Times YouTube, NYT Business, 무역킹 Trade King YouTube
...
- keyword `china` / publish `2026-08-14` / priority `92.0`
...
- source_names: Financial Times Home, Financial Times World, Reuters Markets via Google News RSS
- sample_headlines:
...
예를 들어 `China tightens grip on Europe’s car supply chain` 같은 제목은 출발점일 뿐입니다. 중요한 건 이 뉴스가 실제로 어떤 자산을 먼저 움직였는지입니다.

## 본문

## 1. 지금 무슨 일이 있었나

이번 글의 출발점은 `China tightens grip on Europe’s car supply chain`입니다. 다만 제목만 읽고 끝내면 가장 중요한 부분을 놓치기 쉽습니다.
...
- 주요 참고 소스: Financial Times Home, Financial Times World, Reuters Markets via Google News RSS

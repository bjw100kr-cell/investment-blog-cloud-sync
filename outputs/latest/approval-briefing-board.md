...
- keyword `bitcoin` / publish `2026-09-11` / priority `122.0`
...
- evidence score: demand `4900` / fallback `source_snapshot_rank` / format `crypto_analysis`
- source_names: CoinDesk RSS, Cointelegraph, Investing.com Crypto News, MarketWatch Breaking News, NYT Business
- sample_headlines:
...
- recent_evidence:
  ...
  - Investing.com Crypto News | 2026-09-09 19:18:42 | Bitcoin hugs support below key MAs as bears take control: Live
- image_slots:
...
2026년 9월 10일 기준, 이 이슈를 그냥 뉴스 하나로 넘기기엔 아깝습니다.
...
예를 들어 `U.S. Treasury sanctions another widespread cyber-scam hub, Xinbi Guarantee` 같은 제목은 출발점일 뿐입니다. 중요한 건 이 뉴스가 실제로 어떤 자산을 먼저 움직였는지입니다.

## 본문

## 1. 지금 무슨 일이 있었나

이번 글의 출발점은 `U.S. Treasury sanctions another widespread cyber-scam hub, Xinbi Guarantee`입니다. 다만 제목만 읽고 끝내면 가장 중요한 부분을 놓치기 쉽습니다.
...
- 주요 참고 소스: CoinDesk RSS, Cointelegraph, Investing.com Crypto News, MarketWatch Breaking News, NYT Business
...
- keyword `us_index_flow` / publish `2026-09-12` / priority `98.0`
...
- source_names: Cointelegraph, Reuters Markets via Google News RSS, 무역킹 Trade King YouTube
- sample_headlines:
...
예를 들어 `Bitcoin fails to reclaim $80K as Bessent fuels yen strength around 153 per dollar` 같은 제목은 출발점일 뿐입니다. 중요한 건 이 뉴스가 실제로 어떤 자산을 먼저 움직였는지입니다.

## 본문

## 1. 지금 무슨 일이 있었나

이번 글의 출발점은 `Bitcoin fails to reclaim $80K as Bessent fuels yen strength around 153 per dollar`입니다. 다만 제목만 읽고 끝내면 가장 중요한 부분을 놓치기 쉽습니다.
...
- 주요 참고 소스: Cointelegraph, Reuters Markets via Google News RSS, 무역킹 Trade King YouTube
...
- keyword `tariffs_trade` / publish `2026-09-13` / priority `93.0`
- review `approve` score `100` / quality `review_before_publish` / ready_now `False`
- intent: 당일 이슈가 내 투자에 어떤 영향을 주는지 빠르게 이해하고 싶은 독자
- CTA focus: 실적·공급망·대표 종목 글로 연결
...
예를 들어 `Trump promises $5,000 checks if Republicans win the midterms. He also teased tariff and ‘DOGE’ dividends last year that never came.` 같은 제목은 출발점일 뿐입니다. 중요한 건 이 뉴스가 실제로 어떤 자산을 먼저 움직였는지입니다.

## 본문

## 1. 지금 무슨 일이 있었나

이번 글의 출발점은 `Trump promises $5,000 checks if Republicans win the midterms. He also teased tariff and ‘DOGE’ dividends last year that never came.`입니다. 다만 제목만 읽고 끝내면 가장 중요한 부분을 놓치기 쉽습니다.
...
- 주요 참고 소스: Financial Times World, MarketWatch Breaking News, NYT Business

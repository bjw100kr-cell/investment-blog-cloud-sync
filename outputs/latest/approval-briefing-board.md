...
- evidence score: demand `4800` / fallback `source_snapshot_rank` / format `crypto_analysis`
...
예를 들어 `Crypto Long & Short: To ETH or not to ETH — is SOL the better diversifier?` 같은 제목은 출발점일 뿐입니다. 중요한 건 이 뉴스가 실제로 어떤 자산을 먼저 움직였는지입니다.

## 본문

## 1. 지금 무슨 일이 있었나

이번 글의 출발점은 `Crypto Long & Short: To ETH or not to ETH — is SOL the better diversifier?`입니다. 다만 제목만 읽고 끝내면 가장 중요한 부분을 놓치기 쉽습니다.
...
- keyword `ai_semiconductors` / publish `2026-07-17` / priority `102.0`
- review `approve` score `100` / quality `pass` / ready_now `True`
- intent: 당일 이슈가 내 투자에 어떤 영향을 주는지 빠르게 이해하고 싶은 독자
...
- source_names: Financial Times YouTube, MarketWatch Breaking News, NYT Business, Reuters Markets via Google News RSS
- sample_headlines:
...
- recent_evidence:
  ...
  - Reuters Markets via Google News RSS | 2026-07-15T05:07:00+00:00 | ASML capacity upgrade soothes AI chip bottleneck fears - Reuters
- image_slots:
...
예를 들어 `Wall St edges higher with earnings in focus; chip stocks retreat - Reuters` 같은 제목은 출발점일 뿐입니다. 중요한 건 이 뉴스가 실제로 어떤 자산을 먼저 움직였는지입니다.

## 본문

## 1. 지금 무슨 일이 있었나

이번 글의 출발점은 `Wall St edges higher with earnings in focus; chip stocks retreat - Reuters`입니다. 다만 제목만 읽고 끝내면 가장 중요한 부분을 놓치기 쉽습니다.
...
- 주요 참고 소스: Financial Times YouTube, MarketWatch Breaking News, NYT Business, Reuters Markets via Google News RSS
...
- keyword `china` / publish `2026-07-18` / priority `89.0`
- review `approve` score `100` / quality `pass` / ready_now `True`
- intent: 당일 이슈가 내 투자에 어떤 영향을 주는지 빠르게 이해하고 싶은 독자
- CTA focus: 실적·공급망·대표 종목 글로 연결
- reason: 복수 소스 교차 확인 가능 (3개), 섹터/세계 흐름 연결 해설 가능
- evidence score: demand `0` / fallback `mapped_candidate` / format `macro_explainer`
- source_names: NYT Business, Reuters Markets via Google News RSS, 무역킹 Trade King YouTube
- sample_headlines:
...
쉽게 말해 해외 주요 매체 보도, 유튜브 해설를 같이 보면 headline 뒤에 있는 자금 흐름과 심리 변화를 더 빨리 잡을 수 있습니다.
예를 들어 `VIEW China's second-quarter economic growth misses market forecast - Reuters` 같은 제목은 출발점일 뿐입니다. 중요한 건 이 뉴스가 실제로 어떤 자산을 먼저 움직였는지입니다.

## 본문

## 1. 지금 무슨 일이 있었나

이번 글의 출발점은 `VIEW China's second-quarter economic growth misses market forecast - Reuters`입니다. 다만 제목만 읽고 끝내면 가장 중요한 부분을 놓치기 쉽습니다.
...
- 주요 참고 소스: NYT Business, Reuters Markets via Google News RSS, 무역킹 Trade King YouTube

...
- keyword `bitcoin` / publish `2026-09-25` / priority `119.0`
...
- evidence score: demand `4600` / fallback `source_snapshot_rank` / format `crypto_analysis`
...
- recent_evidence:
  ...
  - Investing.com Crypto News | 2026-09-24 07:10:54 | Bitcoin slips below 20 SMA at $84,076: Live levels
- image_slots:
...
예를 들어 `Crypto casino Duelbits goes offline after $7 million hot wallet hack` 같은 제목은 출발점일 뿐입니다. 중요한 건 이 뉴스가 실제로 어떤 자산을 먼저 움직였는지입니다.

## 본문

## 1. 지금 무슨 일이 있었나

이번 글의 출발점은 `Crypto casino Duelbits goes offline after $7 million hot wallet hack`입니다. 다만 제목만 읽고 끝내면 가장 중요한 부분을 놓치기 쉽습니다.
...
- keyword `ai_semiconductors` / publish `2026-09-26` / priority `80.0`
- review `approve` score `100` / quality `pass` / ready_now `True`
- intent: 당일 이슈가 내 투자에 어떤 영향을 주는지 빠르게 이해하고 싶은 독자
...
- source_names: CoinDesk RSS, Financial Times YouTube
- sample_headlines:
...
예를 들어 `Cathie Wood's ARK teams with Securitize to tokenize venture fund with OpenAI, Anthropic stakes` 같은 제목은 출발점일 뿐입니다. 중요한 건 이 뉴스가 실제로 어떤 자산을 먼저 움직였는지입니다.

## 본문

## 1. 지금 무슨 일이 있었나

이번 글의 출발점은 `Cathie Wood's ARK teams with Securitize to tokenize venture fund with OpenAI, Anthropic stakes`입니다. 다만 제목만 읽고 끝내면 가장 중요한 부분을 놓치기 쉽습니다.
...
- 주요 참고 소스: CoinDesk RSS, Financial Times YouTube
...
예를 들어 `Trump, Xi to talk Taiwan, AI, trade and Iran as DC summit kicks off` 같은 제목은 출발점일 뿐입니다. 중요한 건 이 뉴스가 실제로 어떤 자산을 먼저 움직였는지입니다.

## 본문

## 1. 지금 무슨 일이 있었나

이번 글의 출발점은 `Trump, Xi to talk Taiwan, AI, trade and Iran as DC summit kicks off`입니다. 다만 제목만 읽고 끝내면 가장 중요한 부분을 놓치기 쉽습니다.
...
- 주요 참고 소스: CNBC Top News, Financial Times Home, NYT Business

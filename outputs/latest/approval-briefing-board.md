...
- keyword `fomc` / publish `2026-08-28` / priority `140.0`
...
- evidence score: demand `5700` / fallback `source_snapshot_rank` / format `macro_explainer`
...
- 주요 참고 소스: CoinDesk RSS, Cointelegraph, Federal Reserve Monetary Policy Press, Financial Times Home, Financial Times World, MarketWatch Breaking News, NYT Business, Reuters Markets via Google News RSS
...
- keyword `bitcoin` / publish `2026-08-29` / priority `121.0`
- review `approve` score `100` / quality `pass` / ready_now `True`
- intent: 당일 이슈가 내 투자에 어떤 영향을 주는지 빠르게 이해하고 싶은 독자
...
- recent_evidence:
  ...
  - Investing.com Crypto News | 2026-08-28 21:37:51 | Bitcoin sheds more than 3% on hawkish Warsh, loses steam after debasement rally
- image_slots:
...
예를 들어 `Ethena looks beyond crypto to squeeze yield from booming equity perpetuals` 같은 제목은 출발점일 뿐입니다. 중요한 건 이 뉴스가 실제로 어떤 자산을 먼저 움직였는지입니다.

## 본문

## 1. 지금 무슨 일이 있었나

이번 글의 출발점은 `Ethena looks beyond crypto to squeeze yield from booming equity perpetuals`입니다. 다만 제목만 읽고 끝내면 가장 중요한 부분을 놓치기 쉽습니다.
...
- keyword `us_big_tech` / publish `2026-08-30` / priority `101.0`
...
예를 들어 `apple watch ultra 4` 같은 제목은 출발점일 뿐입니다. 중요한 건 이 뉴스가 실제로 어떤 자산을 먼저 움직였는지입니다.

## 본문

## 1. 지금 무슨 일이 있었나

이번 글의 출발점은 `apple watch ultra 4`입니다. 다만 제목만 읽고 끝내면 가장 중요한 부분을 놓치기 쉽습니다.
...
- 주요 참고 소스: CNBC Top News, Google Trends US, MarketWatch Breaking News

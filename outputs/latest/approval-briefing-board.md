...
- keyword `fomc` / publish `2026-09-23` / priority `128.0`
...
- keyword `bitcoin` / publish `2026-09-24` / priority `124.0`
- review `approve` score `100` / quality `pass` / ready_now `True`
- intent: 당일 이슈가 내 투자에 어떤 영향을 주는지 빠르게 이해하고 싶은 독자
...
- recent_evidence:
  ...
  - Investing.com Crypto News | 2026-09-22 22:16:25 | Bitcoin takes a breather, halting a big rally that briefly took it above $87k
- image_slots:
...
예를 들어 `Zcash leads crypto majors with a 10% gain as bitcoin holds near $87,000` 같은 제목은 출발점일 뿐입니다. 중요한 건 이 뉴스가 실제로 어떤 자산을 먼저 움직였는지입니다.

## 본문

## 1. 지금 무슨 일이 있었나

이번 글의 출발점은 `Zcash leads crypto majors with a 10% gain as bitcoin holds near $87,000`입니다. 다만 제목만 읽고 끝내면 가장 중요한 부분을 놓치기 쉽습니다.
...
- keyword `us_index_flow` / publish `2026-09-25` / priority `105.0`
...
- source_names: Financial Times Home, Financial Times World, MarketWatch Breaking News, Reuters Markets via Google News RSS
- sample_headlines:
...
- recent_evidence:
  ...
  - Financial Times Home | 2026-09-22T20:28:13+00:00 | Nasdaq 100 rises to new high as ‘AI Fomo’ returns
- image_slots:
...
예를 들어 `Nasdaq 100 rises to new high as ‘AI Fomo’ returns` 같은 제목은 출발점일 뿐입니다. 중요한 건 이 뉴스가 실제로 어떤 자산을 먼저 움직였는지입니다.

## 본문

## 1. 지금 무슨 일이 있었나

이번 글의 출발점은 `Nasdaq 100 rises to new high as ‘AI Fomo’ returns`입니다. 다만 제목만 읽고 끝내면 가장 중요한 부분을 놓치기 쉽습니다.
...
- 주요 참고 소스: Financial Times Home, Financial Times World, MarketWatch Breaking News, Reuters Markets via Google News RSS

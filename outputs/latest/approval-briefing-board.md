...
- keyword `bitcoin` / publish `2026-07-25` / priority `122.0`
...
- recent_evidence:
  ...
  - Investing.com Crypto News | 2026-07-24 13:48:08 | Bitcoin ticks down to $64.4k on Iran tensions, US tariff jitters
- image_slots:
...
2026년 7월 24일 기준, 이 이슈를 그냥 뉴스 하나로 넘기기엔 아깝습니다.
...
- keyword `china` / publish `2026-07-27` / priority `111.0`
...
- evidence score: demand `2800` / fallback `source_snapshot_rank` / format `macro_explainer`
...
- recent_evidence:
  ...
  - NYT Business | 2026-07-24T14:20:13+00:00 | China Has Come Through Trump’s Trade War in a Good Position
- image_slots:
...
예를 들어 `Yang Zhilin, the rock star founder behind China’s Moonshot AI` 같은 제목은 출발점일 뿐입니다. 중요한 건 이 뉴스가 실제로 어떤 자산을 먼저 움직였는지입니다.

## 본문

## 1. 지금 무슨 일이 있었나

이번 글의 출발점은 `Yang Zhilin, the rock star founder behind China’s Moonshot AI`입니다. 다만 제목만 읽고 끝내면 가장 중요한 부분을 놓치기 쉽습니다.
...
- keyword `ai_semiconductors` / publish `2026-07-26` / priority `104.0`
- review `approve` score `100` / quality `review_before_publish` / ready_now `False`
- intent: 당일 이슈가 내 투자에 어떤 영향을 주는지 빠르게 이해하고 싶은 독자
- CTA focus: 실적·공급망·대표 종목 글로 연결
- reason: 복수 소스 교차 확인 가능 (4개), 섹터/세계 흐름 연결 해설 가능
...
예를 들어 `Anthropic's new AI model rivals Fable 5 and is cheaper as businesses fret about costs` 같은 제목은 출발점일 뿐입니다. 중요한 건 이 뉴스가 실제로 어떤 자산을 먼저 움직였는지입니다.

## 본문

## 1. 지금 무슨 일이 있었나

이번 글의 출발점은 `Anthropic's new AI model rivals Fable 5 and is cheaper as businesses fret about costs`입니다. 다만 제목만 읽고 끝내면 가장 중요한 부분을 놓치기 쉽습니다.

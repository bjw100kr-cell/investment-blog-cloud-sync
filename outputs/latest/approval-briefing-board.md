...
- keyword `fomc` / publish `2026-07-02` / priority `137.0`
...
- evidence score: demand `4600` / fallback `source_snapshot_rank` / format `macro_explainer`
...
- 주요 참고 소스: CoinDesk RSS, Cointelegraph, Federal Reserve Monetary Policy Press, NYT Business, Reuters Markets via Google News RSS
...
- keyword `bitcoin` / publish `2026-07-03` / priority `127.0`
- review `approve` score `100` / quality `pass` / ready_now `True`
- intent: 당일 이슈가 내 투자에 어떤 영향을 주는지 빠르게 이해하고 싶은 독자
...
- recent_evidence:
  ...
  - CoinDesk RSS | 2026-07-02T12:10:15+00:00 | A struggling Nasdaq-listed company that tried to copy Saylor's Bitcoin playbook is completely dumping crypto for AI
- image_slots:
...
예를 들어 `Bitwise says STRC selloff signals crypto cycle nearing a bottom, not Strategy’s breaking point` 같은 제목은 출발점일 뿐입니다. 중요한 건 이 뉴스가 실제로 어떤 자산을 먼저 움직였는지입니다.

## 본문

## 1. 지금 무슨 일이 있었나

이번 글의 출발점은 `Bitwise says STRC selloff signals crypto cycle nearing a bottom, not Strategy’s breaking point`입니다. 다만 제목만 읽고 끝내면 가장 중요한 부분을 놓치기 쉽습니다.
...
- keyword `us_big_tech` / publish `` / priority `103.0`
...
- recent_evidence:
  ...
  - CNBC Top News | 2026-07-02T13:00:02+00:00 | Microsoft commits $2.5 billion and 6,000 employees to new AI implementation unit
- image_slots:
...
예를 들어 `Tesla stock sinks 7% despite strong deliveries report` 같은 제목은 출발점일 뿐입니다. 중요한 건 이 뉴스가 실제로 어떤 자산을 먼저 움직였는지입니다.

## 본문

## 1. 지금 무슨 일이 있었나

이번 글의 출발점은 `Tesla stock sinks 7% despite strong deliveries report`입니다. 다만 제목만 읽고 끝내면 가장 중요한 부분을 놓치기 쉽습니다.

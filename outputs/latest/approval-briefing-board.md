...
- keyword `fomc` / publish `2026-08-09` / priority `137.0`
...
- evidence score: demand `4500` / fallback `source_snapshot_rank` / format `macro_explainer`
- source_names: Federal Reserve Monetary Policy Press, Investing.com Crypto News, NYT Business, Reuters Markets via Google News RSS
- sample_headlines:
...
2026년 8월 9일 기준, 이 이슈를 그냥 뉴스 하나로 넘기기엔 아깝습니다.
...
- 주요 참고 소스: Federal Reserve Monetary Policy Press, Investing.com Crypto News, NYT Business, Reuters Markets via Google News RSS
...
- keyword `bitcoin` / publish `2026-08-10` / priority `126.0`
- review `approve` score `100` / quality `pass` / ready_now `True`
- intent: 당일 이슈가 내 투자에 어떤 영향을 주는지 빠르게 이해하고 싶은 독자
...
- recent_evidence:
  ...
  - CoinDesk RSS | 2026-08-08T14:00:00+00:00 | Trillions in institutional money to flow into bitcoin, says Bitwise's Matt Hougan
- image_slots:
...
예를 들어 `Bitcoin hits block 961,632 as the controversial BIP-110 soft fork attempt begins` 같은 제목은 출발점일 뿐입니다. 중요한 건 이 뉴스가 실제로 어떤 자산을 먼저 움직였는지입니다.

## 본문

## 1. 지금 무슨 일이 있었나

이번 글의 출발점은 `Bitcoin hits block 961,632 as the controversial BIP-110 soft fork attempt begins`입니다. 다만 제목만 읽고 끝내면 가장 중요한 부분을 놓치기 쉽습니다.
...
- keyword `us_index_flow` / publish `2026-08-11` / priority `106.0`
- review `approve` score `100` / quality `pass` / ready_now `True`
- intent: 당일 이슈가 내 투자에 어떤 영향을 주는지 빠르게 이해하고 싶은 독자
...
- recent_evidence:
  ...
  - Reuters Markets via Google News RSS | 2026-08-07T02:16:00+00:00 | US stocks, bonds rally after soft jobs report; yen bounces back - Reuters
- image_slots:
...
예를 들어 `For retirees, staying in the stock market is critical. How much exposure is the make-or-break question` 같은 제목은 출발점일 뿐입니다. 중요한 건 이 뉴스가 실제로 어떤 자산을 먼저 움직였는지입니다.

## 본문

## 1. 지금 무슨 일이 있었나

이번 글의 출발점은 `For retirees, staying in the stock market is critical. How much exposure is the make-or-break question`입니다. 다만 제목만 읽고 끝내면 가장 중요한 부분을 놓치기 쉽습니다.

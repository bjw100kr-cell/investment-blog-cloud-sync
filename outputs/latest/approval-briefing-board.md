...
- recent_evidence:
  ...
  - CoinDesk RSS | 2026-07-03T05:54:51+00:00 | Finally. $221 million flow into Bitcoin ETFs, ending a painful 10-day outflow streak
- image_slots:
...
예를 들어 `Live updates: Bitcoin rises above $61,000 as the red hot semiconductor trade starts to fade` 같은 제목은 출발점일 뿐입니다. 중요한 건 이 뉴스가 실제로 어떤 자산을 먼저 움직였는지입니다.

## 본문

## 1. 지금 무슨 일이 있었나

이번 글의 출발점은 `Live updates: Bitcoin rises above $61,000 as the red hot semiconductor trade starts to fade`입니다. 다만 제목만 읽고 끝내면 가장 중요한 부분을 놓치기 쉽습니다.
...
- keyword `ai_semiconductors` / publish `2026-07-05` / priority `110.0`
...
- source_names: CoinDesk RSS, Financial Times Home, Financial Times World, Reuters Markets via Google News RSS
- sample_headlines:
  - Live updates: Bitcoin rises above $61,000 as the red hot semiconductor trade starts to fade
...
- 주요 참고 소스: CoinDesk RSS, Financial Times Home, Financial Times World, Reuters Markets via Google News RSS
...
- keyword `china` / publish `2026-07-06` / priority `66.0`
- review `approve` score `100` / quality `review_before_publish` / ready_now `False`
- intent: 당일 이슈가 내 투자에 어떤 영향을 주는지 빠르게 이해하고 싶은 독자
- CTA focus: 실적·공급망·대표 종목 글로 연결

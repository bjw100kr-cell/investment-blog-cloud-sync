...
- keyword `bitcoin` / publish `2026-07-07` / priority `124.0`
...
- recent_evidence:
  ...
  - Cointelegraph | 2026-07-06T10:25:20+00:00 | $60.4K Becomes 'most important area': Five things to know in Bitcoin this week
- image_slots:
...
예를 들어 `Bitmine added another $74 million in ether as Tom Lee bets on Clarity Act boost` 같은 제목은 출발점일 뿐입니다. 중요한 건 이 뉴스가 실제로 어떤 자산을 먼저 움직였는지입니다.

## 본문

## 1. 지금 무슨 일이 있었나

이번 글의 출발점은 `Bitmine added another $74 million in ether as Tom Lee bets on Clarity Act boost`입니다. 다만 제목만 읽고 끝내면 가장 중요한 부분을 놓치기 쉽습니다.
...
- keyword `ai_semiconductors` / publish `2026-07-08` / priority `109.0`
...
- source_names: CNBC Top News, Financial Times Home, MarketWatch Breaking News, Reuters Markets via Google News RSS
- sample_headlines:
...
예를 들어 `TeraWulf shares soar after Anthropic leases data center in Kentucky` 같은 제목은 출발점일 뿐입니다. 중요한 건 이 뉴스가 실제로 어떤 자산을 먼저 움직였는지입니다.

## 본문

## 1. 지금 무슨 일이 있었나

이번 글의 출발점은 `TeraWulf shares soar after Anthropic leases data center in Kentucky`입니다. 다만 제목만 읽고 끝내면 가장 중요한 부분을 놓치기 쉽습니다.
...
- 주요 참고 소스: CNBC Top News, Financial Times Home, MarketWatch Breaking News, Reuters Markets via Google News RSS
...
- keyword `tariffs_trade` / publish `2026-07-09` / priority `71.0`
- review `approve` score `100` / quality `review_before_publish` / ready_now `False`
- intent: 당일 이슈가 내 투자에 어떤 영향을 주는지 빠르게 이해하고 싶은 독자
- CTA focus: 실적·공급망·대표 종목 글로 연결
...
예를 들어 `The Chinese Company That Could Start a Trade War With Europe` 같은 제목은 출발점일 뿐입니다. 중요한 건 이 뉴스가 실제로 어떤 자산을 먼저 움직였는지입니다.

## 본문

## 1. 지금 무슨 일이 있었나

이번 글의 출발점은 `The Chinese Company That Could Start a Trade War With Europe`입니다. 다만 제목만 읽고 끝내면 가장 중요한 부분을 놓치기 쉽습니다.

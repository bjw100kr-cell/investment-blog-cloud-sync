...
- keyword `bitcoin` / publish `2026-07-30` / priority `122.0`
...
2026년 7월 29일 기준, 이 이슈를 그냥 뉴스 하나로 넘기기엔 아깝습니다.
...
예를 들어 `The inside story of how a hike in Hong Kong changed crypto trading forever` 같은 제목은 출발점일 뿐입니다. 중요한 건 이 뉴스가 실제로 어떤 자산을 먼저 움직였는지입니다.

## 본문

## 1. 지금 무슨 일이 있었나

이번 글의 출발점은 `The inside story of how a hike in Hong Kong changed crypto trading forever`입니다. 다만 제목만 읽고 끝내면 가장 중요한 부분을 놓치기 쉽습니다.
...
- keyword `ai_semiconductors` / publish `2026-07-31` / priority `114.0`
- review `approve` score `100` / quality `pass` / ready_now `True`
- intent: 당일 이슈가 내 투자에 어떤 영향을 주는지 빠르게 이해하고 싶은 독자
...
- sample_headlines:
  ...
  - Morning Bid: Chip rout moves into pAnIc territory - reuters.com
- recent_evidence:
  ...
  - Financial Times Home | 2026-07-29T10:21:56+00:00 | ‘Anti-tech’ index shines in global chip rout
- image_slots:
...
예를 들어 `Tech rout roils markets after SK Hynix profits disappoint` 같은 제목은 출발점일 뿐입니다. 중요한 건 이 뉴스가 실제로 어떤 자산을 먼저 움직였는지입니다.

## 본문

## 1. 지금 무슨 일이 있었나

이번 글의 출발점은 `Tech rout roils markets after SK Hynix profits disappoint`입니다. 다만 제목만 읽고 끝내면 가장 중요한 부분을 놓치기 쉽습니다.
...
- keyword `china` / publish `2026-08-01` / priority `96.0`
- review `approve` score `100` / quality `pass` / ready_now `True`
- intent: 당일 이슈가 내 투자에 어떤 영향을 주는지 빠르게 이해하고 싶은 독자
- CTA focus: 실적·공급망·대표 종목 글로 연결
- reason: 복수 소스 교차 확인 가능 (4개), 섹터/세계 흐름 연결 해설 가능
- evidence score: demand `0` / fallback `mapped_candidate` / format `macro_explainer`
- source_names: CNBC Top News, Financial Times Home, NYT Business, 무역킹 Trade King YouTube
- sample_headlines:
...
쉽게 말해 해외 주요 매체 보도, 유튜브 해설를 같이 보면 headline 뒤에 있는 자금 흐름과 심리 변화를 더 빨리 잡을 수 있습니다.
예를 들어 `Nike was once China's sneaker king. Here's why its sales have fallen 30%` 같은 제목은 출발점일 뿐입니다. 중요한 건 이 뉴스가 실제로 어떤 자산을 먼저 움직였는지입니다.

## 본문

## 1. 지금 무슨 일이 있었나

이번 글의 출발점은 `Nike was once China's sneaker king. Here's why its sales have fallen 30%`입니다. 다만 제목만 읽고 끝내면 가장 중요한 부분을 놓치기 쉽습니다.
...
- 주요 참고 소스: CNBC Top News, Financial Times Home, NYT Business, 무역킹 Trade King YouTube

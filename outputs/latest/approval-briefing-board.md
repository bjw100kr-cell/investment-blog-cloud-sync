...
- keyword `bitcoin` / publish `2026-08-07` / priority `124.0`
...
- recent_evidence:
  ...
  - CoinDesk RSS | 2026-08-06T10:43:07+00:00 | Bitcoin, ether benefit as traders seek safety of largest tokens
- image_slots:
...
예를 들어 `JPMorgan says Hyperliquid ETF inflows have stalled as competition mounts` 같은 제목은 출발점일 뿐입니다. 중요한 건 이 뉴스가 실제로 어떤 자산을 먼저 움직였는지입니다.

## 본문

## 1. 지금 무슨 일이 있었나

이번 글의 출발점은 `JPMorgan says Hyperliquid ETF inflows have stalled as competition mounts`입니다. 다만 제목만 읽고 끝내면 가장 중요한 부분을 놓치기 쉽습니다.
...
- keyword `ai_semiconductors` / publish `2026-08-08` / priority `91.0`
- review `approve` score `100` / quality `pass` / ready_now `True`
- intent: 당일 이슈가 내 투자에 어떤 영향을 주는지 빠르게 이해하고 싶은 독자
...
예를 들어 `The deal that saved Intel` 같은 제목은 출발점일 뿐입니다. 중요한 건 이 뉴스가 실제로 어떤 자산을 먼저 움직였는지입니다.

## 본문

## 1. 지금 무슨 일이 있었나

이번 글의 출발점은 `The deal that saved Intel`입니다. 다만 제목만 읽고 끝내면 가장 중요한 부분을 놓치기 쉽습니다.
...
- keyword `tariffs_trade` / publish `2026-08-09` / priority `84.0`
...
- evidence score: demand `400` / fallback `trend_match` / format `macro_explainer`
...
예를 들어 `무역` 같은 제목은 출발점일 뿐입니다. 중요한 건 이 뉴스가 실제로 어떤 자산을 먼저 움직였는지입니다.

## 본문

## 1. 지금 무슨 일이 있었나

이번 글의 출발점은 `무역`입니다. 다만 제목만 읽고 끝내면 가장 중요한 부분을 놓치기 쉽습니다.

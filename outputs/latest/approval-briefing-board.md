...
- keyword `fomc` / publish `2026-09-16` / priority `140.0`
...
- source_names: CNBC Top News, Cointelegraph, Federal Reserve Monetary Policy Press, Investing.com Crypto News, MarketWatch Breaking News, NYT Business, Reuters Markets via Google News RSS
- sample_headlines:
...
- 주요 참고 소스: CNBC Top News, Cointelegraph, Federal Reserve Monetary Policy Press, Investing.com Crypto News, MarketWatch Breaking News, NYT Business, Reuters Markets via Google News RSS
...
- keyword `bitcoin` / publish `2026-09-17` / priority `121.0`
- review `approve` score `100` / quality `pass` / ready_now `True`
- intent: 당일 이슈가 내 투자에 어떤 영향을 주는지 빠르게 이해하고 싶은 독자
- CTA focus: ETF·규제·초보 가이드 글로 연결
- reason: 복수 소스 교차 확인 가능 (4개), 코인 독자 유입과 재방문 가능성, 코인 시장 신호 반영 (mixed)
- evidence score: demand `4500` / fallback `source_snapshot_rank` / format `crypto_analysis`
...
- recent_evidence:
  ...
  - Investing.com Crypto News | 2026-09-16 07:11:51 | Bitcoin breaks down below 200MA at $75,861: Live levels
- image_slots:
...
예를 들어 `U.S. House's tax committee advances crypto tax bill in wake of Clarity Act loss` 같은 제목은 출발점일 뿐입니다. 중요한 건 이 뉴스가 실제로 어떤 자산을 먼저 움직였는지입니다.

## 본문

## 1. 지금 무슨 일이 있었나

이번 글의 출발점은 `U.S. House's tax committee advances crypto tax bill in wake of Clarity Act loss`입니다. 다만 제목만 읽고 끝내면 가장 중요한 부분을 놓치기 쉽습니다.
...
- keyword `ai_semiconductors` / publish `2026-09-18` / priority `96.0`
- review `approve` score `100` / quality `pass` / ready_now `True`
- intent: 당일 이슈가 내 투자에 어떤 영향을 주는지 빠르게 이해하고 싶은 독자
...
예를 들어 `AI bosses’ safety push sparks rift inside OpenAI and Anthropic` 같은 제목은 출발점일 뿐입니다. 중요한 건 이 뉴스가 실제로 어떤 자산을 먼저 움직였는지입니다.

## 본문

## 1. 지금 무슨 일이 있었나

이번 글의 출발점은 `AI bosses’ safety push sparks rift inside OpenAI and Anthropic`입니다. 다만 제목만 읽고 끝내면 가장 중요한 부분을 놓치기 쉽습니다.
...
- 주요 참고 소스: Financial Times Home, Financial Times World, Financial Times YouTube

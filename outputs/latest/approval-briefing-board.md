...
- keyword `bitcoin` / publish `2026-08-05` / priority `124.0`
...
- evidence score: demand `6900` / fallback `source_snapshot_rank` / format `crypto_analysis`
...
- recent_evidence:
  ...
  - Investing.com Crypto News | 2026-08-03 21:34:56 | Bitcoin recovers slightly from Coldcard hack losses, gaining marginally above $63k
- image_slots:
...
2026년 8월 4일 기준, 이 이슈를 그냥 뉴스 하나로 넘기기엔 아깝습니다.
...
예를 들어 `U.S. FBI intelligence agent arrested in connection with theft of $1 million in crypto` 같은 제목은 출발점일 뿐입니다. 중요한 건 이 뉴스가 실제로 어떤 자산을 먼저 움직였는지입니다.

## 본문

## 1. 지금 무슨 일이 있었나

이번 글의 출발점은 `U.S. FBI intelligence agent arrested in connection with theft of $1 million in crypto`입니다. 다만 제목만 읽고 끝내면 가장 중요한 부분을 놓치기 쉽습니다.
...
- keyword `fomc` / publish `2026-08-04` / priority `134.0`
- review `approve` score `100` / quality `pass` / ready_now `True`
- intent: 당일 이슈가 내 투자에 어떤 영향을 주는지 빠르게 이해하고 싶은 독자
...
- 주요 참고 소스: CNBC Top News, Federal Reserve Monetary Policy Press
...
- keyword `ai_growth_stocks` / publish `2026-08-06` / priority `112.0`
...
- recent_evidence:
  ...
  - Financial Times YouTube | 137K views | Palantir: profits, procurement and power | FT Film
- image_slots:
...
```md
# AI 성장주를 볼 때 놓치기 쉬운 리스크: 매출 성장과 금리 부담: 오늘 시장이 반응한 이유와 확인할 3가지

한 줄 요약: `공식 발표 날짜와 핵심 문구`, `달러·금리·주식·코인 중 먼저 반응한 자산`, `거래량과 자금 흐름` 세 지표를 같이 봐야 이 이슈가 단기 뉴스인지 실제 흐름인지 구분할 수 있습니다.

## 오늘 핵심 3줄

- AI 성장주를 볼 때 놓치기 쉬운 리스크: 매출 성장과 금리 부담은 제목만 보면 단순 뉴스처럼 보이지만, 실제로는 자금 흐름과 투자심리를 같이 건드릴 수 있는 이슈입니다.
...
예를 들어 `Palantir soars 12% on blowout quarter, with U.S. commercial revenue soaring nearly 150%` 같은 제목은 출발점일 뿐입니다. 중요한 건 이 뉴스가 실제로 어떤 자산을 먼저 움직였는지입니다.

## 본문

## 1. 지금 무슨 일이 있었나

이번 글의 출발점은 `Palantir soars 12% on blowout quarter, with U.S. commercial revenue soaring nearly 150%`입니다. 다만 제목만 읽고 끝내면 가장 중요한 부분을 놓치기 쉽습니다.
...
- 발행 전 재확인: 팔란티어/AI 성장주 관련 수치와 날짜 재확인
- 발행 전 재확인: 단기 검색 급증과 중장기 펀더멘털을 구분해 서술
- 발행 전 재확인: 섹터 자금 흐름 연결 문장 과장 여부 점검
...
다음 글에서는 AI 성장주를 볼 때 놓치기 쉬운 리스크: 매출 성장과 금리 부담: 오늘 시장이 반응한 이유와 확인할 3가지 흐름이 실제 종목이나 자산군 선택으로 어떻게 이어지는지 더 실전적으로 풀어보겠습니다.

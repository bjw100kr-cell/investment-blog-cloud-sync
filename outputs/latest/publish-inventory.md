# 발행 재고판

- 생성 시각: `2026-08-18T12:59:37.397038+00:00`
- 전체 발행 후보: `9`
- 업로드 가능 글 수: `9`
- 메인 글 수: `4`
- SEO 후속 글 수: `5`

## 1. 비트코인 가격보다 먼저 봐야 할 것: ETF 자금, 달러, 규제 체크포인트

- inventory_type: main_post
- keyword: bitcoin
- source_keyword: bitcoin
- role/type: lane_focus_crypto / breaking_explainer
- ready_to_upload: True
- publish_date: 2026-08-19 / bucket: tomorrow
- priority_score: 122.0
- revenue_objective: 페이지뷰와 체류시간 균형 확보
- search_intent: 당일 이슈가 내 투자에 어떤 영향을 주는지 빠르게 이해하고 싶은 독자
- cta_focus: ETF·규제·초보 가이드 글로 연결

## 2. AI 반도체 주가를 볼 때 실적보다 먼저 확인할 3가지

- inventory_type: main_post
- keyword: ai_semiconductors
- source_keyword: ai_semiconductors
- role/type: lane_focus_us-stocks / breaking_explainer
- ready_to_upload: True
- publish_date: 2026-08-20 / bucket: this_week
- priority_score: 82.0
- revenue_objective: 페이지뷰와 체류시간 균형 확보
- search_intent: 당일 이슈가 내 투자에 어떤 영향을 주는지 빠르게 이해하고 싶은 독자
- cta_focus: 실적·공급망·대표 종목 글로 연결

## 3. 중국 변수와 시장 영향: 환율, 경기부양, 원자재를 같이 봐야 하는 이유

- inventory_type: main_post
- keyword: china
- source_keyword: china
- role/type: lane_focus_world-flow / breaking_explainer
- ready_to_upload: True
- publish_date: 2026-08-21 / bucket: this_week
- priority_score: 79.0
- revenue_objective: 페이지뷰와 체류시간 균형 확보
- search_intent: 당일 이슈가 내 투자에 어떤 영향을 주는지 빠르게 이해하고 싶은 독자
- cta_focus: 실적·공급망·대표 종목 글로 연결

## 4. FOMC 이후 시장, 주식과 코인이 같이 흔들리는 이유와 확인할 3가지

- inventory_type: main_post
- keyword: fomc
- source_keyword: fomc
- role/type: evergreen_support / evergreen_explainer
- ready_to_upload: True
- publish_date: 2026-08-22 / bucket: later
- priority_score: 124.0
- revenue_objective: 페이지뷰와 체류시간 균형 확보
- search_intent: FOMC가 무엇인지와 금리·주식·코인에 왜 중요한지 알고 싶어 하는 초중급 투자자
- cta_focus: 환율·금리·미국증시 evergreen 글로 연결

## 5. 미국채 금리 상승 이유: 지금 투자자가 확인할 체크포인트 5가지

- inventory_type: seo_followup
- keyword: seo_treasury_yields_8
- source_keyword: treasury_yields
- role/type: search_demand_capture / follow_up_analysis
- ready_to_upload: True
- publish_date: 2026-08-18 / bucket: seo_backlog
- priority_score: 112.07
- revenue_objective: 새 검색 수요를 빠르게 받아내고 내부링크로 기존 핵심 글과 연결
- search_intent: `미국채 금리 상승 이유`를 검색한 독자가 시장 영향과 확인 지표를 빠르게 이해하려는 의도
- cta_focus: 관련 허브, 기존 메인 해설, 다음 체크포인트 글로 연결

## 6. cpi: 지금 투자자가 확인할 체크포인트 5가지

- inventory_type: seo_followup
- keyword: seo_cpi_9
- source_keyword: cpi
- role/type: search_demand_capture / follow_up_analysis
- ready_to_upload: True
- publish_date: 2026-08-22 / bucket: seo_backlog
- priority_score: 106.07
- revenue_objective: 새 검색 수요를 빠르게 받아내고 내부링크로 기존 핵심 글과 연결
- search_intent: `cpi`를 검색한 독자가 시장 영향과 확인 지표를 빠르게 이해하려는 의도
- cta_focus: 관련 허브, 기존 메인 해설, 다음 체크포인트 글로 연결

## 7. 중국 변수와 시장 영향 관련 대표 종목 한눈에 보기

- inventory_type: seo_followup
- keyword: seo_china_12
- source_keyword: china
- role/type: evergreen_seo / evergreen_sector_guide
- ready_to_upload: True
- publish_date: 2026-08-21 / bucket: seo_backlog
- priority_score: 78.5
- revenue_objective: 섹터형 검색 유입 누적
- search_intent: 섹터 뉴스는 봤지만 실제 어떤 기업을 같이 봐야 하는지 알고 싶은 독자
- cta_focus: 대표 종목 글과 허브 글 연결

## 8. 중국 변수와 시장 영향 공급망 정리: 누가 수혜를 보나

- inventory_type: seo_followup
- keyword: seo_china_14
- source_keyword: china
- role/type: follow_up / follow_up_analysis
- ready_to_upload: True
- publish_date: 2026-08-21 / bucket: seo_backlog
- priority_score: 75.5
- revenue_objective: 체류시간과 페이지뷰 확대
- search_intent: 테마가 실제 공급망과 실적에 어떻게 연결되는지 알고 싶은 독자
- cta_focus: 실적 해설과 글로벌 섹터 허브 연결

## 9. 중국 변수와 시장 영향 ETF·지수·대표 기업 정리

- inventory_type: seo_followup
- keyword: seo_china_15
- source_keyword: china
- role/type: evergreen_seo / evergreen_sector_guide
- ready_to_upload: True
- publish_date: 2026-08-21 / bucket: seo_backlog
- priority_score: 72.5
- revenue_objective: 광고 노출과 장기 검색 유입 확보
- search_intent: 개별 종목보다 묶음으로 섹터를 이해하고 싶은 독자
- cta_focus: 섹터 허브와 후속 비교 글 연결
